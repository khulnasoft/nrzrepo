...
template.name = `nrz-${os}-${humanizedArchLookup[arch]}`;
template.description = `The ${os}-${humanizedArchLookup[arch]} binary for nrz, a monorepo build system.`;
...
if (os === "windows") {
  fs.copyFileSync(
    path.join(__dirname, "template", "bin", "nrz"),
    path.join(outputPath, "bin", "nrz")
  );
}
fs.copyFileSync(
