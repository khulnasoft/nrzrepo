...
pub async fn login<T: Client + TokenClient + CacheClient>(
    ...
    if !force {
        if let Some(token) = existing_token {
            debug!("found existing nrz token");
...
mod tests {
    ...
    impl LoginServer for MockLoginServer {
        async fn run(
            ...
            login_token
                .set(nrzrepo_vercel_api_mock::EXPECTED_TOKEN.to_string())
                .unwrap();
            Ok(())
        }
    }
    ...
    impl From<MockApiError> for nrzrepo_api_client::Error {
        fn from(error: MockApiError) -> Self {
            match error {
                MockApiError::EmptyToken => nrzrepo_api_client::Error::UnknownStatus {
    ...
    impl Client for MockApiClient {
        async fn get_user(&self, token: &str) -> nrzrepo_api_client::Result<UserResponse> {
            if token.is_empty() {
        ...
        async fn get_teams(&self, token: &str) -> nrzrepo_api_client::Result<TeamsResponse> {
            if token.is_empty() {
                return Err(MockApiError::EmptyToken.into());
            }
        ...
        async fn get_team(
            ...
            _token: &str,
            _team_id: &str,
        ) -> nrzrepo_api_client::Result<Option<Team>> {
        ...
        async fn get_spaces(
            ...
            _token: &str,
            _team_id: Option<&str>,
        ) -> nrzrepo_api_client::Result<SpacesResponse> {
        ...
        async fn verify_sso_token(
            ...
            token: &str,
            _: &str,
        ) -> nrzrepo_api_client::Result<VerifiedSsoUser> {
            Ok(VerifiedSsoUser {
                token: token.to_string(),
                team_id: Some("team_id".to_string()),
            })
        }
        async fn handle_403(_response: Response) -> nrzrepo_api_client::Error {
            unimplemented!("handle_403")
        }
        fn make_url(&self, endpoint: &str) -> nrzrepo_api_client::Result<Url> {
            let url = format!("{}{}", self.base_url, endpoint);
            Url::parse(&url).map_err(|err| nrzrepo_api_client::Error::InvalidUrl { url, err })
        }
    }
    ...
    impl TokenClient for MockApiClient {
        ...
        ) -> nrzrepo_api_client::Result<nrzrepo_vercel_api::token::ResponseTokenMetadata>
        {
            ...
            Ok(nrzrepo_vercel_api::token::ResponseTokenMetadata {
                id: "id".to_string(),
                name: "name".to_string(),
                token_type: "token".to_string(),
                origin: "github".to_string(),
                scopes: vec![nrzrepo_vercel_api::token::Scope {
        ...
        async fn delete_token(&self, _token: &str) -> nrzrepo_api_client::Result<()> {
            Ok(())
        }
    }
    ...
    impl CacheClient for MockApiClient {
        async fn get_artifact(
            ...
            _method: Method,
        ) -> Result<Option<Response>, nrzrepo_api_client::Error> {
            unimplemented!("get_artifact")
        }
        async fn put_artifact(
            &self,
            _hash: &str,
            _artifact_body: impl nrzrepo_api_client::Stream<
                    Item = Result<nrzrepo_api_client::Bytes, nrzrepo_api_client::Error>,
                > + Send
                + Sync
                + 'static,
            _body_len: usize,
            ...
            _team_id: Option<&str>,
            _team_slug: Option<&str>,
        ) -> Result<(), nrzrepo_api_client::Error> {
        ...
        async fn get_caching_status(
            ...
            _team_id: Option<&str>,
            _team_slug: Option<&str>,
        ) -> Result<CachingStatusResponse, nrzrepo_api_client::Error> {
    ...
    async fn test_login() {
        ...
        assert_eq!(&got_token, nrzrepo_vercel_api_mock::EXPECTED_TOKEN);
