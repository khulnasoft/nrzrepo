...
impl<T: TokenClient> LogoutOptions<T> {
    ...
    async fn remove_tokens(&self) -> Result<(), Error> {
        ...
        if let Some(nrz_config_dir) = config_dir()? {
            self.try_remove_token(
                &nrz_config_dir.join_components(&[NRZ_TOKEN_DIR, NRZ_TOKEN_FILE]),
            )
            .await?;
        }
...
mod tests {
    ...
    impl Client for MockApiClient {
        async fn get_user(&self, _token: &str) -> nrzrepo_api_client::Result<UserResponse> {
            unimplemented!("get_user")
        }
        async fn get_teams(&self, _token: &str) -> nrzrepo_api_client::Result<TeamsResponse> {
        ...
        fn make_url(&self, _endpoint: &str) -> nrzrepo_api_client::Result<Url> {
    ...
    impl TokenClient for MockApiClient {
        async fn delete_token(&self, _token: &str) -> nrzrepo_api_client::Result<()> {
            if self.succeed_delete_request {
                ...
                Err(nrzrepo_api_client::Error::UnknownStatus {
                    code: "code".to_string(),
        ...
        async fn get_metadata(
            &self,
            _token: &str,
        ) -> nrzrepo_api_client::Result<ResponseTokenMetadata> {
