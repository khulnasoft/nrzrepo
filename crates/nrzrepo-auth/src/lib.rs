...
#![deny(clippy::all)]
//! Nrzrepo's library for authenticating with the Vercel API.
...
pub const NRZ_TOKEN_DIR: &str = "nrzrepo";
pub const NRZ_TOKEN_FILE: &str = "config.json";
    ...
    pub async fn has_cache_access<T: CacheClient>(
            ...
            .await
        {
            ...
            Err(e) => match e {
                ...
                nrzrepo_api_client::Error::UnknownStatus { code, .. } if code == "forbidden" => {
                    Ok(false)
                }
                // If the entire request fails with 403 also return false
                nrzrepo_api_client::Error::ReqwestError(e)
