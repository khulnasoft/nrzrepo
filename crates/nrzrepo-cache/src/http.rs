...
impl HTTPCache {
    ...
    pub async fn put(
        ...
        .map(|res| {
            res.map(|bytes| bytes.freeze())
                .map_err(nrzrepo_api_client::Error::from)
        });
        ...
        let body = if let Some(signer_verifier) = &self.signer_verifier {
            ...
            let body = response.bytes().await.map_err(|e| {
                CacheError::ApiClientError(
                    Box::new(nrzrepo_api_client::Error::ReqwestError(e)),
                    Backtrace::capture(),
                )
            })?;
    ...
    fn convert_api_error(hash: &str, err: nrzrepo_api_client::Error) -> CacheError {
        match err {
            nrzrepo_api_client::Error::ReqwestError(e) if e.is_timeout() => {
                CacheError::TimeoutError(hash.to_string())
            }
            nrzrepo_api_client::Error::ReqwestError(e) if e.is_connect() => {
...
mod test {
    ...
    fn test_forbidden_error() {
        let err = HTTPCache::convert_api_error(
            "hash",
            nrzrepo_api_client::Error::UnknownStatus {
                code: "forbidden".into(),
    ...
    fn test_cache_disabled() {
        let err = HTTPCache::convert_api_error(
            "hash",
            nrzrepo_api_client::Error::CacheDisabled {
                status: nrzrepo_vercel_api::CachingStatus::Disabled,
