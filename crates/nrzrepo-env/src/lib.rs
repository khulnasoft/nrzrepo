...
mod tests {
    ...
    fn test_case_sensitivity() {
        let start = EnvironmentVariableMap(
            vec![("Nrz".to_string(), "true".to_string())]
                .into_iter()
                .collect(),
        );
        let actual = start.from_wildcards(&["NRZ"]).unwrap();
        if cfg!(windows) {
            assert_eq!(actual.get("Nrz").map(|s| s.as_str()), Some("true"));
        } else {
            assert_eq!(actual.get("Nrz"), None);
        }
    }
