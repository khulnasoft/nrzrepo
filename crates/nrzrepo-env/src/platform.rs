...
use nrzrepo_ui::{ceprint, ceprintln, color, ColorConfig, BOLD, GREY, UNDERLINE, YELLOW};
...
const NRZ_PLATFORM_ENV_KEY: &str = "NRZ_PLATFORM_ENV";
const NRZ_PLATFORM_ENV_DISABLED_KEY: &str = "NRZ_PLATFORM_ENV_DISABLED";

impl PlatformEnv {
    pub fn new() -> Self {
        let env_keys = std::env::var(NRZ_PLATFORM_ENV_KEY)
            .unwrap_or_default()
            .split(',')
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string())
    ...
    pub fn disabled() -> bool {
        let nrz_platform_env_disabled =
            std::env::var(NRZ_PLATFORM_ENV_DISABLED_KEY).unwrap_or_default();
        nrz_platform_env_disabled == "1" || nrz_platform_env_disabled == "true"
    }
            ...
            "VERCEL" => {
                ceprintln!(
                    ...
                    "Warning - the following environment variables are set on your Vercel \
                     project, but missing from \"nrz.json\". {strict_message} Learn more at \
                     {docs_message}\n"
                );
            }
            _ => {
