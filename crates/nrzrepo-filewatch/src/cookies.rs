...
pub struct CookieWriter {
    // Where we put the cookie files, usually `<repo_root>/.nrz/cookies`
...
pub(crate) struct CookieWatcher<T> {
    // Where we expect to find the cookie files, usually `<repo_root>/.nrz/cookies`
    cookie_root: AbsoluteSystemPathBuf,
...
impl CookieWriter {
    ...
    pub fn new_with_default_cookie_dir(
        ...
        let cookie_root = repo_root.join_components(&[".nrz", "cookies"]);
