...
impl Subscriber {
    ...
    async fn handle_workspace_changes(
            ...
            .filter_map(|p| p.as_os_str().to_str())
        {
            ...
            let nrz_json = path_workspace.join_component("nrz.json");

            let (package_exists, nrz_exists) = join!(
            ...
            changed |= if package_exists {
                workspaces
                    .insert(
                        ...
                        WorkspaceData {
                            package_json,
                            nrz_json: nrz_exists.unwrap_or_default().then_some(nrz_json),
                        },
                    )
                    .is_none()
            } else {
...
mod test {
    ...
    use nrzrepo_repository::{discovery::WorkspaceData, package_manager::PackageManager};
