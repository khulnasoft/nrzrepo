...
mod error;
// Global nrz sets this environment variable to its cwd so that local
// nrz can use it for package inference.
pub const INVOCATION_DIR_ENV_VAR: &str = "NRZ_INVOCATION_DIR";
...
impl From<OutputLogsMode> for nrzrepo_ui::tui::event::OutputLogs {
    fn from(value: OutputLogsMode) -> Self {
        match value {
            OutputLogsMode::Full => nrzrepo_ui::tui::event::OutputLogs::Full,
            OutputLogsMode::None => nrzrepo_ui::tui::event::OutputLogs::None,
            OutputLogsMode::HashOnly => nrzrepo_ui::tui::event::OutputLogs::HashOnly,
            OutputLogsMode::NewOnly => nrzrepo_ui::tui::event::OutputLogs::NewOnly,
            OutputLogsMode::ErrorsOnly => nrzrepo_ui::tui::event::OutputLogs::ErrorsOnly,
        }
    }
}
...
pub struct Args {
    ...
    pub version: bool,
    #[clap(long, global = true)]
    /// Skip any attempts to infer which version of Nrz the project is
    /// configured to use
    pub skip_infer: bool,
    ...
    #[clap(long, global = true)]
    pub no_color: bool,
    /// When enabled, nrz will precede HTTP requests with an OPTIONS request
    /// for authorization
    #[clap(long, global = true)]
    pub preflight: bool,
    ...
    /// Force a check for a new version of nrz
    ...
    ///
    /// `nrz` will use hints from codebase to guess which package manager
    /// should be used.
    #[clap(long, global = true)]
    pub dangerously_disable_package_manager_check: bool,
    #[clap(long = "experimental-allow-no-nrz-json", hide = true, global = true)]
    pub allow_no_nrz_json: bool,
    /// Use the `nrz.json` located at the provided path instead of one at the
    /// root of the repository.
    #[clap(long, global = true)]
    pub root_nrz_json: Option<Utf8PathBuf>,
...
pub enum DaemonCommand {
    /// Restarts the nrz daemon
    Restart,
    /// Ensures that the nrz daemon is running
    Start,
    /// Reports the status of the nrz daemon
    Status {
        /// Pass --json to report status in JSON format
        #[clap(long)]
        json: bool,
    },
    /// Stops the nrz daemon
    Stop,
    /// Stops the nrz daemon if it is already running, and removes any stale
    /// daemon state
    Clean {
        /// Clean
    ...
    pub fn track(&self, tel: &GenericEventBuilder) {
        ...
        if let Some(remote_cache_timeout) = self.remote_cache_timeout {
            tel.track_arg_value(
                ...
                nrzrepo_telemetry::events::EventType::NonSensitive,
            );
        }
...
pub enum Command {
    ...
    /// Runs the Nrzrepo background daemon
    Daemon {
        /// Set the idle timeout for nrzd
    ...
    /// Nrz your monorepo by running a number of 'repo lints' to
    ...
    Ls {
        ...
        /// nrz's documentation https://turbo.build/repo/docs/reference/command-line-reference/run#--filter
        #[clap(short = 'F', long, group = "scope-filter-group")]
        filter: Vec<String>,
    ...
    ///
    /// By default, nrz executes tasks in topological order (i.e.
...
pub struct GenerateWorkspaceArgs {
    ...
    /// The root of your repository (default: directory with root nrz.json)
    #[clap(short = 'r', long)]
...
pub struct ExecutionArgs {
    ...
    /// Use "strict" to use an allowlist specified in nrz.json.
    ...
    /// all output. Use "hash-only" to show only nrz-computed
    ...
    /// nrz decide based on its own heuristics. (default auto)
    #[clap(long, value_enum)]
    ...
    /// prefixing. Use "auto" to let nrz decide how to prefix the logs
    ...
    // > Usage: nrz [OPTIONS] [TASKS]... [-- <FORWARDED_ARGS>...] [COMMAND]
    #[clap(hide = true)]
...
pub struct RunArgs {
    ...
    // -----------------------
    /// Force nrz to use the local daemon. If unset
    /// nrz will use the default detection logic.
    #[clap(long, group = "daemon-group")]
    pub daemon: bool,

    /// Force nrz to not use the local daemon. If unset
    /// nrz will use the default detection logic.
    #[clap(long, group = "daemon-group")]
    pub no_daemon: bool,

    /// File to write nrz's performance profile output into.
...
///
/// Scenarios:
/// 1. inference failed, we're running this global nrz. no repo state
/// 2. --skip-infer was passed, assume we're local nrz and run. no repo state
/// 3. There is no local nrz, we're running the global one. repo state exists
/// 4. nrz binary path is set, and it's this one. repo state exists
///
/// # Arguments
///
/// * `repo_state`: If we have done repository inference and NOT executed
/// local nrz, such as in the case where `NRZ_BINARY_PATH` is set,
...
pub async fn run(
    ...
    #[allow(unused_variables)] logger: &NrzSubscriber,
    ...
    let should_print_version = env::var("NRZ_PRINT_VERSION_DISABLED")
        .map_or(true, |disable| !matches!(disable.as_str(), "1" | "true"))
        && !nrzrepo_ci::is_ci();

    if should_print_version {
        eprintln!("{}\n", GREY.apply_to(format!("nrz {}", get_version())));
    }
        ...
        | Command::Watch { execution_args, .. } => {
            ...
            if cli_args.cwd.is_none() {
                if let Ok(invocation_dir) = env::var(INVOCATION_DIR_ENV_VAR) {
                    ...
                    // If repo state doesn't exist, we're either local nrz running at the root
                    // (cwd), or inference failed.
                    // If repo state does exist, we're global nrz, and want to calculate
    ...
    // track system info
    root_telemetry.track_platform(NrzState::platform_name());
    root_telemetry.track_version(NrzState::version());
    ...
    let cli_result = match cli_args.command.as_ref().unwrap() {
        ...
        Command::Completion { shell } => {
            CommandEventBuilder::new("completion")
                .with_parent(&root_telemetry)
                .track_call();
            generate(*shell, &mut Args::command(), "nrz", &mut io::stdout());
            Ok(0)
        }
    };
...
mod test {
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--framework-inference"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                     tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--framework-inference", "true"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--cache-dir", "foobar"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--cache-workers", "100"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec ! ["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--filter", "water", "--filter", "earth", "--filter", "fire", "--filter", "air"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "-F", "water", "-F", "earth", "-F", "fire", "-F", "air"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--filter", "water", "-F", "earth", "--filter", "fire", "-F", "air"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&[ "nrz", "run", "build", "--global-deps", ".env", "--global-deps", ".env.development"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--output-logs", "full"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--output-logs", "none"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--output-logs", "hash-only"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--log-order", "grouped"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--profile", "profile_out"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--remote-only", "true"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["nrz", "run", "build", "--remote-only", "false"],
        Args {
            command: Some(Command::Run {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
        &["nrz", "watch", "build", "--cache-dir", "foobar"],
        Args {
            command: Some(Command::Watch {
                execution_args: Box::new(ExecutionArgs {
                    tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
        &["nrz", "watch", "build", "--experimental-write-cache"],
        Args {
            command: Some(Command::Watch {
                execution_args: Box::new(ExecutionArgs {
                  tasks: vec!["build".to_string()],
    ...
    fn test_parse_bin() {
        assert_eq!(
            Args::try_parse_from(["nrz", "bin"]).unwrap(),
            Args {
    ...
    fn test_parse_link() {
        assert_eq!(
            Args::try_parse_from(["nrz", "link"]).unwrap(),
            Args {
    ...
    fn test_parse_login() {
        assert_eq!(
            Args::try_parse_from(["nrz", "login"]).unwrap(),
            Args {
    ...
    fn test_parse_logout() {
        assert_eq!(
            Args::try_parse_from(["nrz", "logout"]).unwrap(),
            Args {
    ...
    fn test_parse_unlink() {
        assert_eq!(
            Args::try_parse_from(["nrz", "unlink"]).unwrap(),
            Args {
    ...
    fn test_parse_prune() {
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "prune", "foo"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "prune", "--scope", "bar"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "prune", "foo", "bar"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "prune", "--docker", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "prune", "--out-dir", "dist", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
    ...
    fn test_pass_through_args() {
        assert_eq!(
            Args::try_parse_from(["nrz", "run", "build", "--", "--script-arg=42"]).unwrap(),
            Args {
                command: Some(Command::Run {
                    run_args: Box::new(RunArgs {
                        ..get_default_run_args()
    ...
    fn test_parse_prune_no_mixed_arg_and_flag() {
        assert!(Args::try_parse_from(["nrz", "prune", "foo", "--scope", "bar"]).is_err(),);
    }
    ...
    fn test_parse_gen() {
        ...
        assert_eq!(
            Args::try_parse_from(["nrz", "gen"]).unwrap(),
            Args {
    ...
    fn test_profile_usage() {
        assert!(Args::try_parse_from(["nrz", "build", "--profile", ""]).is_err());
        assert!(Args::try_parse_from(["nrz", "build", "--anon-profile", ""]).is_err());
        assert!(Args::try_parse_from(["nrz", "build", "--profile", "foo.json"]).is_ok());
        assert!(Args::try_parse_from(["nrz", "build", "--anon-profile", "foo.json"]).is_ok());
    ...
    fn test_empty_cache_dir() {
        assert!(Args::try_parse_from(["nrz", "build", "--cache-dir"]).is_err());
        assert!(Args::try_parse_from(["nrz", "build", "--cache-dir="]).is_err());
        assert!(Args::try_parse_from(["nrz", "build", "--cache-dir", ""]).is_err());
    }
    ...
    #[test]
    fn test_preflight() {
        assert!(!Args::try_parse_from(["nrz", "build",]).unwrap().preflight);
        assert!(
            Args::try_parse_from(["nrz", "build", "--preflight"])
                .unwrap()
                .preflight
        );
        assert!(Args::try_parse_from(["nrz", "build", "--preflight=true"]).is_err());
    }
    ...
    fn test_prevent_affected_and_filter() {
        ...
        assert!(Args::try_parse_from(["nrz", "ls", "--filter", "foo", "--affected"]).is_err(),);
    }
    ...
    const SINGLE_PKG: SinglePackageTestCase =
        SinglePackageTestCase::new(&["nrz", "--single-package", "run", "build"])
            .expected(&["nrz", "run", "build"]);
    ...
    .expected(&["nrz", "run", "build", "--", "--single-package"]);
    ...
    #[test_case::test_case(&["nrz", "watch", "build", "--no-daemon"]; "after watch")]
    #[test_case::test_case(&["nrz", "--no-daemon", "watch", "build"]; "before watch")]
    ...
    #[test_case::test_case(&["nrz", "--filter=foo", "run", "build"], false; "execution args")]
    #[test_case::test_case(&["nrz", "--no-daemon", "run", "build"], false; "run args")]
    #[test_case::test_case(&["nrz", "build", "run"], true; "task")]
    #[test_case::test_case(&["nrz", "--filter=web", "watch", "build"], false; "execution before watch")]
    ...
    #[test_case::test_case(&["nrz", "--filter=foo", "boundaries"], false; "execution args")]
    #[test_case::test_case(&["nrz", "--no-daemon", "boundaries"], false; "run args")]
