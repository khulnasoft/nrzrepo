...
const DAEMON_NOT_RUNNING_MESSAGE: &str =
    "daemon is not running, run `nrz daemon start` to start it";
...
async fn get_log_file_from_folder(base: &CommandBase) -> Result<String, DaemonError> {
    ...
    let log_folder = base.repo_root.join_components(&[".nrz", "daemon"]);
...
pub async fn daemon_server(
    ...
    let server =
        crate::daemon::NrzGrpcService::new(base.repo_root.clone(), paths, timeout, exit_signal);
...
pub struct DaemonStatus {
    ...
    pub pid_file: nrzpath::AbsoluteSystemPathBuf,
    pub sock_file: nrzpath::AbsoluteSystemPathBuf,
}
