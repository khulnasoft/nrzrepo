...
fn call_nrz_gen(command: &str, tag: &String, raw_args: &str) -> Result<i32, Error> {
    debug!(
        "Running @nrz/gen@{} with command `{}` and args {:?}",
        tag, command, raw_args
    );
    let npx_path = which("npx").map_err(Error::NpxNotFound)?;
    let mut npx = Command::new(npx_path);
    npx.arg("--yes")
        .arg(format!("@nrz/gen@{}", tag))
        .arg("raw")
        .arg(command)
        .args(["--json", raw_args])
...
pub fn run(
    ...
    if let Some(box GenerateCommand::Workspace(workspace_args)) = command {
        ...
        call_nrz_gen("workspace", tag, &raw_args)?;
    } else {
