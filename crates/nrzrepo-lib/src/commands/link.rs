...
pub enum Error {
    ...
    #[error("User not found. Please login to Nrzrepo first by running {command}.")]
...
fn select_team<'a>(base: &CommandBase, teams: &'a [Team]) -> Result<SelectedTeam<'a>, Error> {
    ...
    let prompt = format!(
        ...
        base.color_config.apply(BOLD.apply_to(
            "Which Vercel scope (and Remote Cache) do you want associated with this Nrzrepo?",
        )),
        base.color_config
...
mod test {
    ...
    async fn test_link_remote_cache() -> Result<()> {
        ...
        let config = NrzrepoConfigBuilder::new(&repo_root)
        ...
        // read the config
        let updated_config = NrzrepoConfigBuilder::new(&base.repo_root)
            .with_global_config_path(override_global_config_path)
            .build()?;
        let team_id = updated_config.team_id();

        assert!(
            team_id == Some(nrzrepo_vercel_api_mock::EXPECTED_USER_ID)
                || team_id == Some(nrzrepo_vercel_api_mock::EXPECTED_TEAM_ID)
        );
    ...
    async fn test_link_spaces() -> Result<()> {
        ...
        assert_eq!(
            ...
            nrzrepo_vercel_api_mock::EXPECTED_SPACE_ID.into()
        );
