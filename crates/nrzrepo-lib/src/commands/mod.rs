...
use nrzrepo_auth::{NRZ_TOKEN_DIR, NRZ_TOKEN_FILE};
use nrzrepo_dirs::config_dir;
use nrzrepo_ui::ColorConfig;

use crate::{
    cli,
    config::{ConfigurationOptions, Error as ConfigError, NrzrepoConfigBuilder},
    opts::Opts,
    Args,
};
...
impl CommandBase {
    ...
    pub fn load_config(
        ...
        NrzrepoConfigBuilder::new(repo_root)
            ...
            .with_allow_no_nrz_json(args.allow_no_nrz_json.then_some(true))
            .build()
    }
    ...
    fn local_config_path(&self) -> AbsoluteSystemPathBuf {
        self.repo_root.join_components(&[".nrz", "config.json"])
    }
    ...
    /// Current working directory for the nrz command
