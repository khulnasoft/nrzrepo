...
pub enum Error {
    ...
    NrzJsonParser(#[from] crate::nrz_json::parser::Error),
    #[error(transparent)]
    PackageJson(#[from] nrzrepo_repository::package_json::Error),
    #[error(transparent)]
    PackageGraph(#[from] package_graph::Error),
    #[error(transparent)]
    Lockfile(#[from] nrzrepo_lockfiles::Error),
    #[error("`nrz` does not support workspaces at file system root.")]
...
fn nrz_json() -> &'static AnchoredSystemPath {
    static PATH: OnceLock<&'static AnchoredSystemPath> = OnceLock::new();
    PATH.get_or_init(|| AnchoredSystemPath::new("nrz.json").unwrap())
}
...
pub async fn prune(
    ...
    if matches!(
        ...
        nrzrepo_repository::package_manager::PackageManager::Bun
    ) {
    ...
    prune.copy_nrz_json(&workspace_names)?;
...
impl<'a> Prune<'a> {
    ...
    fn copy_file(
            ...
            )
        {
            ...
            nrzrepo_fs::copy_file(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_directory(
        ...
        let full_to = self.full_directory.resolve(path);
        nrzrepo_fs::recursive_copy(&from_path, full_to, self.use_gitignore)?;
        if matches!(destination, Some(CopyDestination::All)) {
            let out_to = self.out_directory.resolve(path);
            nrzrepo_fs::recursive_copy(&from_path, out_to, self.use_gitignore)?;
        }
        if self.docker
            && matches!(
                destination,
        ...
        {
            let docker_to = self.docker_directory().resolve(path);
            nrzrepo_fs::recursive_copy(&from_path, docker_to, self.use_gitignore)?;
        }
        Ok(())
    }
    ...
    fn copy_workspace(&self, package_json_path: &AnchoredSystemPath) -> Result<(), Error> {
        ...
        nrzrepo_fs::recursive_copy(original_dir, &target_dir, self.use_gitignore)?;
    ...
    fn copy_nrz_json(&self, workspaces: &[String]) -> Result<(), Error> {
        let anchored_nrz_path = nrz_json();
        let original_nrz_path = self.root.resolve(anchored_nrz_path);
        let new_nrz_path = self.full_directory.resolve(anchored_nrz_path);

        let nrz_json_contents = match original_nrz_path.read_to_string() {
            Ok(contents) => contents,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                // If nrz.json doesn't exist skip copying
                return Ok(());
            }
            Err(e) => return Err(e.into()),
        };

        let nrz_json = RawNrzJson::parse(&nrz_json_contents, anchored_nrz_path.as_str())?;

        let pruned_nrz_json = nrz_json.prune_tasks(workspaces);
        new_nrz_path.create_with_contents(serde_json::to_string_pretty(&pruned_nrz_json)?)?;
