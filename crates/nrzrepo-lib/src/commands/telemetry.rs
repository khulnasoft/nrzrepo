use nrzrepo_telemetry::{config::TelemetryConfig, events::command::CommandEventBuilder};
use nrzrepo_ui::{color, BOLD, BOLD_GREEN, BOLD_RED};
...
fn log_status(config: TelemetryConfig, base: &CommandBase) {
    ...
    match status {
        ...
        false => {
            ...
            println!(
                "\nYou have opted-out of Nrzrepo anonymous telemetry. No data will be collected \
                 from your machine."
            );
        }
    }
