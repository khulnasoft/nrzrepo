...
use nrzrepo_auth::{NRZ_TOKEN_DIR, NRZ_TOKEN_FILE, VERCEL_TOKEN_DIR, VERCEL_TOKEN_FILE};
...
impl ResolvedConfigurationOptions for AuthFile {
    fn get_configuration_options(
        ...
        // No auth token found in either Vercel or Nrz config.
...
fn global_auth_path() -> Result<AbsoluteSystemPathBuf, Error> {
    ...
    // Check for both Vercel and Nrz paths. Vercel takes priority.
