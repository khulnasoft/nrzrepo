...
#[diagnostic(
    ...
    url("{}/messages/{}", NRZ_SITE, self.code().unwrap().to_string().to_case(Case::Kebab))
)]
...
pub enum Error {
    ...
    NoGlobalConfigDir,
    #[error(transparent)]
    PackageJson(#[from] nrzrepo_repository::package_json::Error),
    #[error(
        "Could not find nrz.json.\nFollow directions at https://turbo.build/repo/docs to create \
         one."
    )]
    NoNrzJSON,
    #[error(transparent)]
    ...
    #[error(transparent)]
    Cache(#[from] nrzrepo_cache::config::Error),
    #[error(
        "Package tasks (<package>#<task>) are not allowed in single-package repositories: found \
         {task_id}"
    )]
    #[diagnostic(code(package_task_in_single_package_mode), url("{}/messages/{}", NRZ_SITE, self.code().unwrap().to_string().to_case(Case::Kebab)))]
    ...
    #[error(transparent)]
    PathError(#[from] nrzpath::PathError),
    #[diagnostic(
    ...
    ApiClient(#[source] nrzrepo_api_client::Error),
    #[error("{0} is not UTF8.")]
    Encoding(String),
    #[error("NRZ_SIGNATURE should be either 1 or 0.")]
    InvalidSignature,
    #[error("NRZ_REMOTE_CACHE_ENABLED should be either 1 or 0.")]
    InvalidRemoteCacheEnabled,
    #[error("NRZ_REMOTE_CACHE_TIMEOUT: Error parsing timeout.")]
    InvalidRemoteCacheTimeout(#[source] std::num::ParseIntError),
    #[error("NRZ_REMOTE_CACHE_UPLOAD_TIMEOUT: Error parsing timeout.")]
    InvalidUploadTimeout(#[source] std::num::ParseIntError),
    #[error("NRZ_PREFLIGHT should be either 1 or 0.")]
    InvalidPreflight,
    #[error("NRZ_LOG_ORDER should be one of: {0}")]
    InvalidLogOrder(String),
    #[error(transparent)]
    #[diagnostic(transparent)]
    NrzJsonParseError(#[from] crate::nrz_json::parser::Error),
    #[error("found absolute path in `cacheDir`")]
    #[diagnostic(help("If absolute paths are required, use `--cache-dir` or `NRZ_CACHE_DIR`."))]
    AbsoluteCacheDir {
    ...
    #[error("Cannot load nrz.json for {0} in single package mode.")]
    InvalidNrzJsonLoad(PackageName),
}
...
#[setters(
    prefix = "with_",
    generate_delegates(ty = "NrzrepoConfigBuilder", field = "override_config")
)]
pub struct ConfigurationOptions {
    ...
    #[serde(alias = "TeamSlug")]
    #[serde(alias = "TEAMSLUG")]
    /// corresponds to env var NRZ_TEAM
    pub(crate) team_slug: Option<String>,
    #[serde(alias = "teamid")]
    #[serde(alias = "TeamId")]
    #[serde(alias = "TEAMID")]
    /// corresponds to env var NRZ_TEAMID
    pub(crate) team_id: Option<String>,
    /// corresponds to env var NRZ_TOKEN
    ...
    #[serde(skip)]
    pub(crate) root_nrz_json_path: Option<AbsoluteSystemPathBuf>,
    ...
    pub(crate) allow_no_nrz_json: Option<bool>,
}

#[derive(Default)]
pub struct NrzrepoConfigBuilder {
...
impl ConfigurationOptions {
    ...
    pub fn root_nrz_json_path(&self, repo_root: &AbsoluteSystemPath) -> AbsoluteSystemPathBuf {
        self.root_nrz_json_path
            .clone()
            .unwrap_or_else(|| repo_root.join_component(CONFIG_FILE))
    }

    pub fn allow_no_nrz_json(&self) -> bool {
        self.allow_no_nrz_json.unwrap_or_default()
    }
}
...
impl NrzrepoConfigBuilder {
    ...
    #[allow(dead_code)]
    fn root_nrz_json_path(&self) -> AbsoluteSystemPathBuf {
        self.repo_root.join_component("nrz.json")
    }
    ...
    pub fn build(&self) -> Result<ConfigurationOptions, Error> {
        ...
        // - shared configuration (nrz.json)
        // - global configuration (~/.nrz/config.json)
        // - local configuration (<REPO_ROOT>/.nrz/config.json)
        // - environment variables
        // - CLI arguments
        // - builder pattern overrides.

        let nrz_json = NrzJsonReader::new(&self.repo_root);
...
mod test {
    ...
    fn test_defaults() {
        ...
        .unwrap();
        assert_eq!(
            defaults.root_nrz_json_path(repo_root),
            repo_root.join_component("nrz.json")
        )
    }
    ...
    fn test_env_layering() {
        ...
        let builder = NrzrepoConfigBuilder {
            repo_root,
            override_config: Default::default(),
        ...
        assert_eq!(config.team_id().unwrap(), nrz_teamid);
        assert_eq!(config.token().unwrap(), nrz_token);
        assert_eq!(config.spaces_id().unwrap(), "my-spaces-id");
    }

    #[test]
    fn test_nrz_json_remote_cache() {
        let tmp_dir = TempDir::new().unwrap();
        let repo_root = AbsoluteSystemPathBuf::try_from(tmp_dir.path()).unwrap();
        ...
        let team_id = "an-id";
        let nrz_json_contents = serde_json::to_string_pretty(&serde_json::json!({
            "remoteCache": {
                "enabled": true,
                "apiUrl": api_url,
                "loginUrl": login_url,
        ...
        repo_root
            .join_component("nrz.json")
            .create_with_contents(&nrz_json_contents)
            .unwrap();
