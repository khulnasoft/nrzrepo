...
pub struct DaemonClient<T> {
    client: proto::nrzd_client::NrzdClient<tonic::transport::Channel>,
    connect_settings: T,
}

impl DaemonClient<()> {
    pub fn new(client: proto::nrzd_client::NrzdClient<tonic::transport::Channel>) -> Self {
        Self {
            client,
            connect_settings: (),
        }
...
pub enum DaemonError {
    ...
    /// The server is running a different version of nrzrepo.
    #[error("version mismatch: {0}")]
    #[diagnostic(help("try restarting the daemon with `nrz daemon restart`."))]
    ...
    PackageManager(#[from] nrzrepo_repository::package_manager::Error),
...
mod test {
    ...
    fn test_format_repo_relative_glob() {
        let raw_glob = ["some", ".nrz", "nrz-foo:bar.log"].join(MAIN_SEPARATOR_STR);
        #[cfg(windows)]
        let expected = "some/.nrz/nrz-foo";
        #[cfg(not(windows))]
        let expected = "some/.nrz/nrz-foo\\:bar.log";
