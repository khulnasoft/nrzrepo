...
impl DaemonConnector {
    ...
    async fn get_connection(
        &self,
        path: nrzpath::AbsoluteSystemPathBuf,
    ) -> Result<NrzdClient<tonic::transport::Channel>, DaemonConnectorError> {
...
#[cfg(target_os = "windows")]
fn win(
    path: Arc<nrzpath::AbsoluteSystemPathBuf>,
...
pub enum FileWaitError {
    ...
    InvalidPath(nrzpath::AbsoluteSystemPathBuf),
}
...
mod test {
    ...
    #[tonic::async_trait]
    impl proto::nrzd_server::Nrzd for DummyServer {
        async fn shutdown(
            &self,
    ...
    async fn handles_kill_live_server() {
        ...
        let service = ServiceBuilder::new().layer(DefaultTimeoutLayer).service(
            proto::nrzd_server::NrzdServer::new(DummyServer {
