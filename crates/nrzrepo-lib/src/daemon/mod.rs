//! The Nrzrepo daemon watches files and pre-computes data to speed up nrz's
...
fn daemon_log_file_and_folder(
    ...
    let log_folder = repo_root.join_components(&[".nrz", "daemon"]);
    let log_file = log_folder.join_component(format!("{}-nrz.log", repo_hash).as_str());
...
impl Paths {
    pub fn from_repo_root(repo_root: &AbsoluteSystemPath) -> Self {
        ...
        Self {
            pid_file: daemon_root.join_component("nrzd.pid"),
            lock_file: daemon_root.join_component("nrzd.lock"),
            sock_file: daemon_root.join_component("nrzd.sock"),
...
pub(crate) mod proto {
    ...
    tonic::include_proto!("nrzdprotocol");
    ...
    impl From<PackageManager> for nrzrepo_repository::package_manager::PackageManager {
    ...
    impl From<nrzrepo_repository::package_manager::PackageManager> for PackageManager {
        fn from(pm: nrzrepo_repository::package_manager::PackageManager) -> Self {
            match pm {
                nrzrepo_repository::package_manager::PackageManager::Npm => Self::Npm,
                nrzrepo_repository::package_manager::PackageManager::Yarn => Self::Yarn,
                nrzrepo_repository::package_manager::PackageManager::Berry => Self::Berry,
                nrzrepo_repository::package_manager::PackageManager::Pnpm => Self::Pnpm,
                nrzrepo_repository::package_manager::PackageManager::Pnpm6 => Self::Pnpm6,
                nrzrepo_repository::package_manager::PackageManager::Pnpm9 => Self::Pnpm9,
                nrzrepo_repository::package_manager::PackageManager::Bun => Self::Bun,
            }
        }
    }
}
...
mod test {
    ...
    fn test_repo_hash() {
        ...
        let (path, expected_hash) = ("/tmp/nrzrepo", "6e0cfa616f75a61c");
        #[cfg(target_os = "windows")]
        let (path, expected_hash) = ("C:\\\\tmp\\nrzrepo", "0103736e6883e35f");
