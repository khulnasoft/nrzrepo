...
impl FileWatching {
    /// This function is called in the constructor for the `NrzGrpcService`
...
impl<S> NrzGrpcService<S>
where
    S: Future<Output = CloseReason>,
{
    /// Create a gRPC server providing the Nrzd interface. external_shutdown
    ...
    pub async fn serve(self) -> Result<CloseReason, package_manager::Error> {
        ...
        let (service, exit_root_watch, watch_root_handle) =
            NrzGrpcServiceInner::new(repo_root.clone(), trigger_shutdown, paths.log_file);
        ...
        // Now that the server has exited, the NrzGrpcService instance should be
...
#[tonic::async_trait]
impl proto::nrzd_server::Nrzd for NrzGrpcServiceInner {
    async fn hello(
        &self,
            ...
            Some(Ok(packages)) => Ok(tonic::Response::new(proto::DiscoverPackagesResponse {
                package_files: packages
                    ...
                    .map(|d| proto::PackageFiles {
                        ...
                        nrz_json: d.nrz_json.map(|t| t.to_string()),
                    })
                    .collect(),
...
impl NamedService for NrzGrpcServiceInner {
    const NAME: &'static str = "nrzrepo.Daemon";
}
...
mod test {
    ...
    async fn lifecycle() {
        ...
        let service = NrzGrpcService::new(
            repo_root.clone(),
            paths.clone(),
