...
impl Diagnostic for DaemonDiagnostic {
    ...
    fn execute(&self, chan: DiagnosticChannel) {
        ...
        tokio::task::spawn(async move {
            chan.started("Nrz Daemon".to_string()).await;
        ...
        tokio::task::spawn(async move {
            ...
            match pidlock.get_owner() {
                ...
                Ok(None) => {
                    ...
                    chan.failed("Nrzrepo Extension is not running".to_string())
                        .await;
                }
                Err(e) => {
...
impl Diagnostic for UpdateDiagnostic {
    ...
    fn execute(&self, chan: DiagnosticChannel) {
        ...
        tokio::task::spawn(async move {
            chan.started("Update Nrzrepo to latest version".to_string())
                .await;
            ...
            match version {
                Ok(Ok(Some(version))) => {
                    chan.log_line(format!("Nrzrepo {} is available", version).to_string())
                        .await;
                    ...
                    match resp.await.as_ref().map(|s| s.as_str()) {
                        Ok("Yes") => {
                            chan.log_line("Updating Nrzrepo...".to_string()).await;
                            let mut command = Command::new("npx");
                            let command = command
                                .arg("--yes")
                                .arg("@nrz/codemod@latest")
                                .arg("update")
                                .arg("--force")
                            ...
                            match command.wait_with_output().await {
                                Ok(output) if output.status.success() => {
                                    chan.done("Nrzrepo on latest version".to_string()).await;
                                }
                                Ok(output) => {
                                    chan.log_line(
