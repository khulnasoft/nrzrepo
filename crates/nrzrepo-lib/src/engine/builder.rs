...
use nrzrepo_errors::{Spanned, NRZ_SITE};
use nrzrepo_graph_utils as graph;
use nrzrepo_repository::package_graph::{PackageGraph, PackageName, PackageNode, ROOT_PKG_NAME};
...
pub enum Error {
    ...
    #[error(
        "{task_id} requires an entry in nrz.json before it can be depended on because it is a \
         task declared in the root package.json"
    )]
    #[diagnostic(
        code(missing_root_task_in_nrz_json),
        url(
            "{}/messages/{}",
            NRZ_SITE, self.code().unwrap().to_string().to_case(Case::Kebab)
        )
    )]
    MissingRootTaskInNrzJson {
        task_id: String,
        #[label("Add an entry in nrz.json for this task")]
        span: Option<SourceSpan>,
        #[source_code]
        text: NamedSource,
    },
    ...
    #[error("Could not find \"{task_id}\" in root nrz.json or \"{task_name}\" in package")]
    MissingPackageTask {
        #[label]
        span: Option<SourceSpan>,
        #[source_code]
    ...
    #[error("Invalid nrz.json configuration")]
    Validation {
        #[related]
...
pub struct EngineBuilder<'a> {
    ...
    nrz_json_loader: Option<NrzJsonLoader>,
    is_single: bool,
...
impl<'a> EngineBuilder<'a> {
    pub fn new(
        repo_root: &'a AbsoluteSystemPath,
        package_graph: &'a PackageGraph,
        nrz_json_loader: NrzJsonLoader,
        is_single: bool,
    ) -> Self {
        Self {
            repo_root,
            package_graph,
            nrz_json_loader: Some(nrz_json_loader),
            is_single,
            workspaces: Vec::new(),
            tasks: Vec::new(),
    ...
    pub fn build(mut self) -> Result<super::Engine, Error> {
        ...
        let mut nrz_json_loader = self
            .nrz_json_loader
            .take()
            .expect("engine builder cannot be constructed without NrzJsonLoader");
        ...
        let tasks: Vec<Spanned<TaskName<'static>>> = if self.add_all_tasks {
            ...
            for workspace in self.workspaces.iter() {
                let Ok(nrz_json) = nrz_json_loader.load(workspace) else {
                    continue;
                };
        ...
        for (workspace, task) in self.workspaces.iter().cartesian_product(tasks.iter()) {
            ...
            if Self::has_task_definition(&mut nrz_json_loader, workspace, task, &task_id)? {
            ...
            let errors = missing_tasks
                ...
                .map(|(name, span)| {
                    if let Some(pkg) = missing_pkgs.get(&name) {
                        ...
                        let (span, text) = span.span_and_text("nrz.json");
        ...
        while let Some(task_id) = traversal_queue.pop_front() {
            ...
            {
                let (span, text) = task_id.span_and_text("nrz.json");
                return Err(Error::MissingRootTaskInNrzJson {
                    span,
                    text,
    ...
    fn has_task_definition(
        loader: &mut NrzJsonLoader,
        workspace: &PackageName,
        task_name: &TaskName<'static>,
        task_id: &TaskId,
    ) -> Result<bool, Error> {
        let nrz_json = loader.load(workspace).map_or_else(
            |err| {
                if matches!(err, config::Error::NoNrzJSON)
            ...
            |nrz_json| Ok(Some(nrz_json)),
        )?;

        let Some(nrz_json) = nrz_json else {
            // If there was no nrz.json in the workspace, fallback to the root nrz.json
            return Self::has_task_definition(loader, &PackageName::Root, task_name, task_id);
        };

        let task_id_as_name = task_id.as_task_name();
        if
        // See if pkg#task is defined e.g. `docs#build`. This can only happen in root nrz.json
        nrz_json.tasks.contains_key(&task_id_as_name)
            // See if task is defined e.g. `build`. This can happen in root or workspace nrz.json
            // This will fail if the user provided a task id e.g. nrz `docs#build`
            || nrz_json.tasks.contains_key(task_name)
            // If user provided a task id, then we see if the task is defined
            // e.g. `docs#build` should resolve if there's a `build` in root nrz.json or docs workspace level nrz.json
            || (matches!(workspace, PackageName::Root) && nrz_json.tasks.contains_key(&TaskName::from(task_name.task())))
            || (workspace == &PackageName::from(task_id.package()) && nrz_json.tasks.contains_key(&TaskName::from(task_name.task())))
        {
            Ok(true)
    ...
    fn task_definition(
        &self,
        nrz_json_loader: &mut NrzJsonLoader,
    ...
    fn task_definition_chain(
        ...
        let root_nrz_json = nrz_json_loader.load(&PackageName::Root)?;

        if let Some(root_definition) = root_nrz_json.task(task_id, task_name) {
        ...
        if self.is_single {
            return match task_definitions.is_empty() {
                true => {
                    let (span, text) = task_id.span_and_text("nrz.json");
                    Err(Error::MissingRootTaskInNrzJson {
                        span,
                        text,
                        task_id: task_id.to_string(),
                    })
        ...
        if task_id.package() != ROOT_PKG_NAME {
            match nrz_json_loader.load(&PackageName::from(task_id.package())) {
                Ok(workspace_json) => {
...
impl Error {
    fn is_missing_nrz_json(&self) -> bool {
        matches!(self, Self::Config(crate::config::Error::NoNrzJSON))
    }
}
...
fn validate_task_name(task: Spanned<&str>) -> Result<(), Error> {
    INVALID_TOKENS
        ...
        .map(|found_token| {
            let (span, text) = task.span_and_text("nrz.json");
...
mod test {
    ...
    impl Lockfile for MockLockfile {
        fn resolve_package(
            ...
            _name: &str,
            _version: &str,
        ) -> Result<Option<nrzrepo_lockfiles::Package>, nrzrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn all_dependencies(
            &self,
            _key: &str,
        ) -> Result<Option<HashMap<String, String>>, nrzrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> Result<Box<dyn Lockfile>, nrzrepo_lockfiles::Error> {
            unreachable!()
        }

        fn encode(&self) -> Result<Vec<u8>, nrzrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn nrz_version(&self) -> Option<String> {
            None
        }
    }
    ...
    impl PackageDiscovery for MockDiscovery {
        async fn discover_packages(
            ...
            nrzrepo_repository::discovery::DiscoveryResponse,
            nrzrepo_repository::discovery::Error,
        > {
            Ok(nrzrepo_repository::discovery::DiscoveryResponse {
    ...
    fn nrz_json(value: serde_json::Value) -> NrzJson {
        let json_text = serde_json::to_string(&value).unwrap();
        let raw = RawNrzJson::parse(&json_text, "").unwrap();
        NrzJson::try_from(raw).unwrap()
    }
    ...
    fn test_task_definition(
        ...
        .into_iter()
        .collect();
        let mut loader = NrzJsonLoader::noop(nrz_jsons);
    ...
    fn test_default_engine() {
        ...
        .into_iter()
        .collect();
        let loader = NrzJsonLoader::noop(nrz_jsons);
    ...
    fn test_depend_on_missing_task() {
        ...
        assert_matches!(engine, Err(Error::MissingRootTaskInNrzJson { .. }));
    }
    ...
    fn test_sibling_task() {
        ...
        let nrz_jsons = vec![(PackageName::Root, {
            let mut t_json = nrz_json(json!({
                "tasks": {
