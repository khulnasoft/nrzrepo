                    ...
                    .neighbors_directed(node_index, petgraph::Direction::Outgoing)
                {
                        ...
                        && package_json.scripts.contains_key(dep_id.task())
                    {
                        let (span, text) = self
                            .task_locations
                            .get(dep_id)
                            .map(|spanned| spanned.span_and_text("nrz.json"))
...
pub enum ValidateError {
    ...
    #[error(
        "You have {persistent_count} persistent tasks but `nrz` is configured for concurrency \
    ...
    #[error(
        "Cannot run interactive task \"{task}\" without Terminal UI. Set `\"ui\": true` in \
         `nrz.json`, use the `--ui=tui` flag, or set `NRZ_UI=true` as an environment variable."
    )]
    ...
    impl<'a> PackageDiscovery for DummyDiscovery<'a> {
        ...
        > {
            ...
            Ok(DiscoveryResponse {
                package_manager: nrzrepo_repository::package_manager::PackageManager::Pnpm,
                workspaces,
            })
        }
