//! hash module
//!
//! This module contains the hash functions used by nrzrepo for certain
...
pub struct TaskHashable<'a> {
    ...
    // task
    pub(crate) package_dir: Option<nrzpath::RelativeUnixPathBuf>,
...
pub struct GlobalHashable<'a> {
    ...
    pub global_file_hash_map: &'a HashMap<nrzpath::RelativeUnixPathBuf, String>,
...
pub struct LockFilePackages(pub Vec<nrzrepo_lockfiles::Package>);

#[derive(Debug, Clone)]
pub struct FileHashes(pub HashMap<nrzpath::RelativeUnixPathBuf, String>);
...
impl From<LockFilePackages> for Builder<HeapAllocator> {
    fn from(LockFilePackages(packages): LockFilePackages) -> Self {
...

        {
            ...
            for (i, nrzrepo_lockfiles::Package { key, version }) in packages.iter().enumerate() {
...
mod test {
    ...
    fn task_hashable() {
        let task_hashable = TaskHashable {
            ...
            package_dir: Some(nrzpath::RelativeUnixPathBuf::new("package_dir").unwrap()),
    ...
    fn global_hashable() {
        let global_file_hash_map = vec![(
            nrzpath::RelativeUnixPathBuf::new("global_file_hash_map").unwrap(),
    ...
    fn file_hashes(pairs: Vec<(String, String)>, expected: &str) {
        let file_hashes = FileHashes(
            pairs
                .into_iter()
                .map(|(a, b)| (nrzpath::RelativeUnixPathBuf::new(a).unwrap(), b))
                .collect(),
        );
