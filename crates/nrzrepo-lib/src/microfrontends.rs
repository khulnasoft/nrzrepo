...
impl MicrofrontendsConfigs {
    ...
    pub fn update_nrz_json(
        &self,
        package_name: &PackageName,
        mut nrz_json: Result<NrzJson, config::Error>,
    ) -> Result<NrzJson, config::Error> {
        ...
        // instead of `nrz.json`s which currently isn't feasible.
        if matches!(package_name, PackageName::Root) {
            if let Ok(nrz_json) = &mut nrz_json {
                nrz_json
                    .global_deps
        ...
        if let Some(FindResult { dev, proxy, .. }) = self.package_nrz_json_update(package_name) {
            // We need to modify nrz.json, use default one if there isn't one present
            let mut nrz_json = nrz_json.or_else(|err| match err {
                config::Error::NoNrzJSON => Ok(NrzJson::default()),
                err => Err(err),
            })?;

            // If the current package contains the proxy task, then add that definition
            if proxy.package() == package_name.as_str() {
                nrz_json.with_proxy(self.mfe_package);
            }

            if let Some(dev) = dev {
                // If this package has a dev task that's part of the MFE, then we make sure the
                // proxy gets included in the task graph.
                nrz_json.with_sibling(
    ...
    fn new<'a>(
        ...
        for (package_name, config) in sorted_packages {
            ...
            let Some(config) = config.or_else(|err| match err {
                nrzrepo_microfrontends::Error::UnsupportedVersion(_) => {
                    unsupported_version.push((package_name.to_string(), err.to_string()));
                    Ok(None)
                }
                nrzrepo_microfrontends::Error::ChildConfig { reference } => {
...
mod test {
    ...
    fn test_configs_added_as_global_deps() {
        ...
        let nrz_json = NrzJson::default();
        let actual = configs
            .update_nrz_json(&PackageName::Root, Ok(nrz_json))
            .unwrap();
