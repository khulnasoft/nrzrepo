...
// A nrz.json config that is synthesized but not yet resolved.
// This means that we've done the work to synthesize the config from
// package.json, but we haven't yet resolved the workspace
// nrz.json files into a single definition. Therefore we keep the
...
#[serde(rename_all = "camelCase")]
// The raw deserialized nrz.json file.
pub struct RawNrzJson {
    #[serde(skip)]
    span: Spanned<()>,
...
pub struct RawTaskDefinition {
    ...
    // instead of deriving them from a NrzJson
    #[serde(skip)]
    env_mode: Option<EnvMode>,
    // This can currently only be set internally and isn't a part of nrz.json
    #[serde(skip)]
...
pub const CONFIG_FILE: &str = "nrz.json";
        ...
        for glob in outputs {
            if let Some(stripped_glob) = glob.value.strip_prefix('!') {
                if Utf8Path::new(stripped_glob).is_absolute() {
                    let (span, text) = glob.span_and_text("nrz.json");
...
impl TryFrom<RawTaskDefinition> for TaskDefinition {
    ...
    fn try_from(raw_task: RawTaskDefinition) -> Result<Self, Error> {
        ...
        if let Some(interactive) = raw_task.interactive {
            let (span, text) = interactive.span_and_text("nrz.json");
        ...
        if *interruptible && !persistent {
            let (span, text) = interruptible.span_and_text("nrz.json");
        ...
        if let Some(depends_on) = raw_task.depends_on {
            for dependency in depends_on.into_inner() {
                let (span, text) = dependency.span_and_text("nrz.json");
        ...
        let inputs = raw_task
            ...
            .map(|input| {
                if Utf8Path::new(&input.value).is_absolute() {
                    let (span, text) = input.span_and_text("nrz.json");
...
impl RawNrzJson {
    pub(crate) fn read(
        ...
        let raw_nrz_json = RawNrzJson::parse(&contents, &root_relative_path)?;

        Ok(raw_nrz_json)
    }

    /// Produces a new nrz.json without any tasks that reference non-existent
    /// workspaces
...
impl TryFrom<RawNrzJson> for NrzJson {
    type Error = Error;

    fn try_from(raw_nrz: RawNrzJson) -> Result<Self, Error> {
        if let Some(pipeline) = raw_nrz.pipeline {
            let (span, text) = pipeline.span_and_text("nrz.json");
            return Err(Error::PipelineField { span, text });
        }
        let mut global_env = HashSet::new();
        let mut global_file_dependencies = HashSet::new();

        if let Some(global_env_from_nrz) = raw_nrz.global_env {
            gather_env_vars(global_env_from_nrz, "globalEnv", &mut global_env)?;
        }

        for global_dep in raw_nrz.global_dependencies.into_iter().flatten() {
            if global_dep.strip_prefix(ENV_PIPELINE_DELIMITER).is_some() {
                let (span, text) = global_dep.span_and_text("nrz.json");
                return Err(Error::InvalidDependsOnValue {
        ...
        Ok(NrzJson {
            text: raw_nrz.span.text,
            path: raw_nrz.span.path,
            global_env: {
                let mut global_env: Vec<_> = global_env.into_iter().collect();
                global_env.sort();
                global_env
            },
            global_pass_through_env: raw_nrz
            ...
            tasks: raw_nrz.tasks.unwrap_or_default(),
...
impl NrzJson {
    ...
    /// Reads a `RawNrzJson` from the given path
    /// and then converts it into `NrzJson`
    pub(crate) fn read(
        repo_root: &AbsoluteSystemPath,
        path: &AbsoluteSystemPath,
    ) -> Result<NrzJson, Error> {
        let raw_nrz_json = RawNrzJson::read(repo_root, path)?;
        raw_nrz_json.try_into()
    }
    ...
    pub fn validate(&self, validations: &[NrzJSONValidation]) -> Vec<Error> {
        validations
            .iter()
    ...
    /// Adds a local proxy task to a workspace NrzJson
...
type NrzJSONValidation = fn(&NrzJson) -> Vec<Error>;

pub fn validate_no_package_task_syntax(nrz_json: &NrzJson) -> Vec<Error> {
    nrz_json
        .tasks
        .iter()
        .filter(|(task_name, _)| task_name.is_package_task())
        .map(|(task_name, entry)| {
            let (span, text) = entry.span_and_text("nrz.json");
...
pub fn validate_extends(nrz_json: &NrzJson) -> Vec<Error> {
    match nrz_json.extends.first() {
        Some(package_name) if package_name != ROOT_PKG_NAME || nrz_json.extends.len() > 1 => {
            let (span, text) = nrz_json.extends.span_and_text("nrz.json");
...
mod tests {
    ...
    #[test]
    fn test_nrz_task_pruning() {
        let json = RawNrzJson::parse_from_serde(json!({
            "tasks": {
                "//#top": {},
                "build": {},
                "a#build": {},
        ...
        let expected: RawNrzJson = RawNrzJson::parse_from_serde(json!({
            "tasks": {
                "//#top": {},
                "build": {},
                "a#build": {},
    ...
    fn test_parsing_output_logs_mode(output_logs: &str, expected: Option<OutputLogsMode>) {
        let json: Result<RawNrzJson, _> = RawNrzJson::parse_from_serde(json!({
            "tasks": {
                "build": {
    ...
    fn test_ui(json: &str, expected: Option<UIMode>) {
        let json = RawNrzJson::parse(json, "").unwrap();
    ...
    fn test_daemon(json: &str, expected: &str) {
        let parsed = RawNrzJson::parse(json, "").unwrap();
    ...
    fn test_ui_serialization(input: &str, expected: &str) {
        let parsed = RawNrzJson::parse(input, "").unwrap();
        let actual = serde_json::to_string(&parsed).unwrap();
        assert_eq!(actual, expected);
    }
    ...
    fn test_allow_no_package_manager_serde(json_str: &str, expected: Option<bool>) {
        let json = RawNrzJson::parse(json_str, "").unwrap();
