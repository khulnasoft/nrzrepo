...
#[error("Failed to parse nrz.json.")]
#[diagnostic(code(nrz_json_parse_error))]
pub struct Error {
    #[related]
...
impl WithMetadata for RawNrzJson {
...
impl RawNrzJson {
    ...
    #[cfg(test)]
    pub fn parse_from_serde(value: serde_json::Value) -> Result<RawNrzJson, Error> {
        let json_string = serde_json::to_string(&value).expect("should be able to serialize");
        Self::parse(&json_string, "nrz.json")
    }
    /// Parses a nrz.json file into the raw representation with span info
    /// attached.
    ///
    /// # Arguments
    ///
    /// * `text`: The text contents of the nrz.json file
    /// * `file_path`: The path to the nrz.json file. Just used for error
    ///   display, so doesn't need to actually be a correct path.
    ///
    /// returns: Result<RawNrzJson, Error>
    pub fn parse(text: &str, file_path: &str) -> Result<RawNrzJson, Error> {
        let result = deserialize_from_json_str::<RawNrzJson>(
            text,
        ...
        let mut nrz_json = result.into_deserialized().ok_or_else(|| Error {
            diagnostics: vec![],
            backtrace: backtrace::Backtrace::capture(),
        })?;

        nrz_json.add_text(Arc::from(text));
        nrz_json.add_path(Arc::from(file_path));
