...
#[derive(Debug, Clone, Serialize)]
pub struct RepoOpts {
    pub root_nrz_json_path: AbsoluteSystemPathBuf,
    pub allow_no_package_manager: bool,
    pub allow_no_nrz_json: bool,
}

/// The fully resolved options for Nrzrepo. This is the combination of config,
...
impl Opts {
    pub fn synthesize_command(&self) -> String {
        let mut cmd = format!("nrz run {}", self.run_opts.tasks.join(" "));
...
impl<'a> From<OptsInputs<'a>> for RepoOpts {
    fn from(inputs: OptsInputs<'a>) -> Self {
        let root_nrz_json_path = inputs.config.root_nrz_json_path(inputs.repo_root);
        let allow_no_package_manager = inputs.config.allow_no_package_manager();
        let allow_no_nrz_json = inputs.config.allow_no_nrz_json();
...
impl<'a> TryFrom<OptsInputs<'a>> for RunOpts {
    ...
    fn try_from(inputs: OptsInputs) -> Result<Self, Self::Error> {
        ...
        let (is_github_actions, log_order, log_prefix) = match inputs.config.log_order() {
            LogOrder::Auto if nrzrepo_ci::Vendor::get_constant() == Some("GITHUB_ACTIONS") => (
                true,
...
impl<'a> TryFrom<OptsInputs<'a>> for CacheOpts {
    ...
    fn try_from(inputs: OptsInputs<'a>) -> Result<Self, Self::Error> {
        let is_linked = nrzrepo_api_client::is_linked(inputs.api_auth);
...
mod test {
    ...
    #[test_case(
        ...
        "nrz run build --filter=my-app -- -v --foo=bar"
    )]
    #[test_case(
        TestCaseOpts{
        ...
        "nrz run build --filter=other-app --filter=my-app -- -v --foo=bar"
    )]
    #[test_case(
        TestCaseOpts{
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nrz run build --filter=my-app --parallel --continue"
    )]
    #[test_case(
        TestCaseOpts{
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nrz run build --filter=my-app --dry"
    )]
    #[test_case(
        TestCaseOpts{
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nrz run build --filter=my-app --dry=json"
    )]
    #[test_case(
        TestCaseOpts{
            filter_patterns: vec!["my-app".to_string()],
        ...
        "nrz run build --filter=my-app --affected"
    )]
    #[test_case(
        TestCaseOpts{
            tasks: vec!["build".to_string()],
    ...
    fn test_synthesize_command(opts_input: TestCaseOpts, expected: &str) {
        ...
        let root_nrz_json_path = config.root_nrz_json_path(&AbsoluteSystemPathBuf::default());

        let opts = Opts {
            repo_opts: RepoOpts {
                root_nrz_json_path,
                allow_no_package_manager: false,
                allow_no_nrz_json: false,
            },
    ...
    #[test_case(
        vec!["nrz", "ls", "--filter", "foo"];
        "ls"
    )]
    #[test_case(
        vec!["nrz", "boundaries", "--filter", "foo"];
        "boundaries"
    )]
