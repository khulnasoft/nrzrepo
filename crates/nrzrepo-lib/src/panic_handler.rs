...
const OPEN_ISSUE_MESSAGE: &str =
    "Please open an issue at https://github.com/khulnasoft/nrzrepo/issues/new/choose";
...
pub fn panic_handler(panic_info: &std::panic::PanicHookInfo) {
    ...
    let report = Report::new("nrz", get_version(), Method::Panic, explanation, cause);
    // If we're in CI we don't persist the backtrace to a temp file as this is hard
    // to retrieve.
    let should_persist = !nrzrepo_ci::is_ci() && nrzrepo_ci::Vendor::infer().is_none();
