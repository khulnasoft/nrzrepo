...
impl From<nrz_trace::TraceError> for Diagnostic {
    fn from(error: nrz_trace::TraceError) -> Self {
        let message = error.to_string();
        match error {
            nrz_trace::TraceError::FileNotFound(file) => Diagnostic {
                message,
                path: Some(file.to_string()),
                ..Default::default()
            },
            nrz_trace::TraceError::PathEncoding(_) => Diagnostic {
                message,
                ..Default::default()
            },
            nrz_trace::TraceError::RootFile(path) => Diagnostic {
                message,
                path: Some(path.to_string()),
                ..Default::default()
            },
            nrz_trace::TraceError::ParseError(path, e) => Diagnostic {
                message: format!("failed to parse file: {:?}", e),
                path: Some(path.to_string()),
                ..Default::default()
            },
            nrz_trace::TraceError::GlobError(err) => Diagnostic {
                message: format!("failed to glob files: {}", err),
                ..Default::default()
            },
            nrz_trace::TraceError::Resolve {
                span,
                text,
                file_path,
                reason,
...
impl TraceResult {
    fn new(result: nrz_trace::TraceResult, run: Arc<Run>) -> Result<Self, Error> {
...
impl From<ImportType> for nrz_trace::ImportTraceType {
    fn from(import_type: ImportType) -> Self {
        match import_type {
            ImportType::All => nrz_trace::ImportTraceType::All,
            ImportType::Types => nrz_trace::ImportTraceType::Types,
            ImportType::Values => nrz_trace::ImportTraceType::Values,
        }
    }
}
