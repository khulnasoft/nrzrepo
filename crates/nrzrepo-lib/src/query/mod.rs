...
impl RepositoryQuery {
    ...
    fn convert_change_reason(
        &self,
        reason: nrzrepo_repository::change_mapper::PackageInclusionReason,
    ) -> PackageChangeReason {
        match reason {
            nrzrepo_repository::change_mapper::PackageInclusionReason::All(
            ...
            nrzrepo_repository::change_mapper::PackageInclusionReason::RootTask { task } => {
                PackageChangeReason::RootTask(RootTask {
                    task_name: task.to_string(),
                })
            }
            nrzrepo_repository::change_mapper::PackageInclusionReason::ConservativeRootLockfileChanged => {
                PackageChangeReason::ConservativeRootLockfileChanged(ConservativeRootLockfileChanged { empty: false })
            }
            nrzrepo_repository::change_mapper::PackageInclusionReason::LockfileChanged { removed, added } => {
            ...
            nrzrepo_repository::change_mapper::PackageInclusionReason::DependencyChanged {
                dependency,
            } => PackageChangeReason::DependencyChanged(DependencyChanged {
                dependency_name: dependency.to_string(),
            }),
            nrzrepo_repository::change_mapper::PackageInclusionReason::DependentChanged {
                dependent,
            } => PackageChangeReason::DependentChanged(DependentChanged {
                dependent_name: dependent.to_string(),
            }),
            nrzrepo_repository::change_mapper::PackageInclusionReason::FileChanged { file } => {
                PackageChangeReason::FileChanged(FileChanged {
                    file_path: file.to_string(),
                })
            }
            nrzrepo_repository::change_mapper::PackageInclusionReason::InFilteredDirectory {
                directory,
            } => PackageChangeReason::InFilteredDirectory(InFilteredDirectory {
                directory_path: directory.to_string(),
            }),
            nrzrepo_repository::change_mapper::PackageInclusionReason::IncludedByFilter {
                filters,
