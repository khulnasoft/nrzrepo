...
impl RunBuilder {
    ...
    pub fn calculate_filtered_packages(
        ...
        if is_all_packages {
            for target in opts.run_opts.tasks.iter() {
                ...
                if root_nrz_json.tasks.contains_key(&task_name) {
    ...
    pub async fn build(
        ...
        let is_linked = nrzrepo_api_client::is_linked(&self.api_auth);
        ...
        let is_ci_or_not_tty = nrzrepo_ci::is_ci() || !std::io::stdout().is_terminal();
        run_telemetry.track_ci(nrzrepo_ci::Vendor::get_name());
        ...
        let daemon = match (is_ci_or_not_tty, self.opts.run_opts.daemon) {
            (true, None) => {
                ...
                debug!("skipping nrzd since we appear to be in a non-interactive context");
                None
            }
            ...
            (_, Some(false)) => {
                ...
                debug!("skipping nrzd since --no-daemon was passed");
                None
            }
        };
        ...
        let mut pkg_dep_graph = {
                ...
                && !self.opts.repo_opts.allow_no_package_manager
            {
                match (&daemon, self.opts.run_opts.daemon) {
                    (None, Some(true)) => {
                        ...
                        return Err(nrzrepo_repository::package_graph::Error::Discovery(
            ...
            match graph {
                ...
                Err(nrzrepo_repository::package_graph::Error::PackageJson(
                    package_json::Error::Io(io),
                )) if io.kind() == ErrorKind::NotFound => {
                    run_telemetry.track_error(TrackedErrors::InvalidPackageDiscovery);
                    return Err(nrzrepo_repository::package_graph::Error::PackageJson(
                        package_json::Error::Io(io),
                    )
                    .into());
                }
        ...
        let mut nrz_json_loader = if task_access.is_enabled() {
            NrzJsonLoader::task_access(
                self.repo_root.clone(),
                self.opts.repo_opts.root_nrz_json_path.clone(),
                root_package_json.clone(),
            )
        } else if is_single_package {
            NrzJsonLoader::single_package(
                self.repo_root.clone(),
                self.opts.repo_opts.root_nrz_json_path.clone(),
                root_package_json.clone(),
            )
        } else if !self.opts.repo_opts.root_nrz_json_path.exists() &&
        // Infer a nrz.json if allowing no nrz.json is explicitly allowed or if MFE configs are discovered
        (self.opts.repo_opts.allow_no_nrz_json || micro_frontend_configs.is_some())
        {
            NrzJsonLoader::workspace_no_nrz_json(
                self.repo_root.clone(),
                pkg_dep_graph.packages(),
                micro_frontend_configs.clone(),
            )
        } else if let Some(micro_frontends) = &micro_frontend_configs {
            NrzJsonLoader::workspace_with_microfrontends(
                self.repo_root.clone(),
                self.opts.repo_opts.root_nrz_json_path.clone(),
                pkg_dep_graph.packages(),
        ...
        let root_nrz_json = nrz_json_loader.load(&PackageName::Root)?.clone();
    ...
    fn build_engine<'a>(
        ...
        .with_root_tasks(root_nrz_json.tasks.keys().cloned())
