...
use nrzrepo_repository::package_graph::{PackageGraph, PackageName, PackageNode};
use nrzrepo_scm::SCM;
use nrzrepo_telemetry::events::generic::GenericEventBuilder;
use nrzrepo_ui::{
...
type UIResult<T> = Result<Option<(T, JoinHandle<Result<(), nrzrepo_ui::Error>>)>, Error>;
...
impl Run {
    ...
    pub fn root_nrz_json(&self) -> &NrzJson {
        &self.root_nrz_json
    }
    ...
    pub async fn run(&self, ui_sender: Option<UISender>, is_watch: bool) -> Result<i32, Error> {
        ...
        if let Some(subscriber) = self.signal_handler.subscribe() {
            ...
            tokio::spawn(async move {
                ...
                let spinner = nrzrepo_ui::start_spinner("...Finishing writing to cache...");
        ...
        let global_hash_inputs = {
            ...
            let pass_through_env = match env_mode {
                ...
                EnvMode::Strict => self.root_nrz_json.global_pass_through_env.as_deref(),
            };
            ...
            get_global_hash_inputs(
                ...
                &self.root_nrz_json.global_deps,
                &self.env_at_execution_start,
                &self.root_nrz_json.global_env,
                pass_through_env,
                env_mode,
