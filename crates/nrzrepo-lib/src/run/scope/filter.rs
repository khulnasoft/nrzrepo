...
impl PackageInference {
    // calculate, based on the directory that global nrz was invoked in,
    // the pieces of a filter spec that we will infer. If nrz was invoked
    // somewhere between the root and packages, scope nrz invocations to the
    // packages below where nrz was invoked. If nrz was invoked at or within
    // a particular package, scope the nrz invocation to just that package.
    pub fn calculate(
        ...
        let full_inference_path = nrz_root.resolve(pkg_inference_path);
        for (workspace_name, workspace_entry) in pkg_graph.packages() {
            let pkg_path = nrz_root.resolve(workspace_entry.package_path());
            ...
            if inferred_path_is_below && (&pkg_path as &AbsoluteSystemPath) != nrz_root {
...
impl<'a> FilterResolver<'a, ScopeChangeDetector<'a>> {
    pub(crate) fn new(
        ...
        inference: Option<PackageInference>,
        scm: &'a SCM,
        root_nrz_json: &'a NrzJson,
    ) -> Result<Self, ResolutionError> {
        let global_deps = opts
            .global_deps
            .iter()
            .map(|s| s.as_str())
            .chain(root_nrz_json.global_deps.iter().map(|s| s.as_str()));

        let change_detector =
            ScopeChangeDetector::new(nrz_root, scm, pkg_graph, global_deps, vec![])?;
...
impl<'a, T: GitChangeDetector> FilterResolver<'a, T> {
    ...
    fn filter_nodes_with_selector(
        ...
        if let Some(globber) = parent_dir_globber.clone() {
            ...
            let path = self.nrz_root.resolve(&base);
            if !path.exists() {
...
pub enum ResolutionError {
    ...
    GlobalDependenciesGlob(#[from] nrzrepo_repository::change_mapper::Error),
}
    ...
    fn make_project<T: GitChangeDetector>(
        ...
        let graph = {
            let rt = tokio::runtime::Builder::new_current_thread()
                ...
                .unwrap();
            rt.block_on(
                PackageGraph::builder(nrz_root, Default::default())
        ...
        let scm = Box::leak(Box::new(nrzrepo_scm::SCM::new(nrz_root)));
