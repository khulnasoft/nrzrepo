...
pub struct ExecutionSummary<'a> {
    // a synthesized nrz command to produce this invocation
    command: String,
    // the (possibly empty) path from the nrzrepo root to where the command was run
...
impl<'a> ExecutionSummary<'a> {
    pub fn new(
        ...
        let duration = NrzDuration::new(&start_time, &end_time);
        Self {
            command,
    ...
    pub fn print(
        ...
        let maybe_full_nrz = if self.cached == self.attempted && self.attempted > 0 {
            match std::env::var("TERM_PROGRAM").as_deref() {
                Ok("Apple_Terminal") => color!(ui, MAGENTA, ">>> FULL NRZ").to_string(),
                _ => ui.rainbow(">>> FULL NRZ").to_string(),
            }
        } else {
            String::new()
        };
