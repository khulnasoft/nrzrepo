...
pub enum Error {
    ...
    SpacesRequest(#[from] nrzrepo_api_client::Error),
...
// for rendering a single-package run of nrz. Notably, we want to always omit
...
impl<'a> From<&'a RunSummary<'a>> for SinglePackageRunSummary<'a> {
    fn from(run_summary: &'a RunSummary<'a>) -> Self {
        ...
        SinglePackageRunSummary {
            ...
            nrz_version: run_summary.nrz_version,
    ...
    async fn send_to_space(
        ...
        let spinner = (!is_watch).then(|| {
            tokio::spawn(async {
                ...
                nrzrepo_ui::start_spinner("...sending run summary...");
            })
        });
