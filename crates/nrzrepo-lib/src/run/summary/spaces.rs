    ...
    fn from(value: SpacesTaskInformation) -> Self {
        ...
        let cache = cache_status.map_or_else(
            ...
            |CacheHitMetadata { source, time_saved }| SpacesCacheStatus {
                status: nrzrepo_api_client::spaces::CacheStatus::Hit,
                source: Some(match source {
                    nrzrepo_cache::CacheSource::Local => {
                        nrzrepo_api_client::spaces::CacheSource::Local
                    }
                    nrzrepo_cache::CacheSource::Remote => {
                        nrzrepo_api_client::spaces::CacheSource::Remote
                    }
                }),
                time_saved,
            },
