...
// trace location
const TASK_ACCESS_ENV_KEY: &str = "NRZREPO_TRACE_FILE";
...
pub const TASK_ACCESS_CONFIG_PATH: [&str; 2] = [".nrz", "traced-config.json"];
/// File name where the task is expected to leave a trace result
const NRZ_CONFIG_FILE: &str = "nrz.json";
...
pub enum ToFileError {
    ...
    Cache(#[from] nrzrepo_cache::CacheError),
}
...
pub fn trace_file_path(
    ...
    repo_root.join_components(&[".nrz", task_hash, TASK_ACCESS_TRACE_NAME])
}
...
fn task_access_trace_enabled(repo_root: &AbsoluteSystemPathBuf) -> Result<bool, std::io::Error> {
    ...
    let root_nrz_json_path = &repo_root.join_component(NRZ_CONFIG_FILE);
    if root_nrz_json_path.exists() {
        return Ok(false);
    }
...
impl TaskAccess {
    pub fn new(repo_root: AbsoluteSystemPathBuf, cache: AsyncCache, scm: &SCM) -> Self {
        ...
        if enabled {
            // make sure .nrz is ignored
            match ensure_nrz_is_gitignored(&repo_root) {
                Ok(_) => debug!("Automatically added .nrz to .gitignore"),
                Err(e) => {
                    error!("Failed to add .nrz to .gitignore. Caching will be disabled - {e}")
                }
            }
    ...
    async fn to_file(&self) -> Result<(), ToFileError> {
        ...
        if let Some(config_cache) = &self.config_cache {
            let traced_config =
                RawNrzJson::from_task_access_trace(&self.trace_by_task.lock().unwrap());
