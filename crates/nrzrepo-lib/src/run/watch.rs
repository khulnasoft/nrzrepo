...
pub struct WatchClient {
    ...
    ui_handle: Option<JoinHandle<Result<(), nrzrepo_ui::Error>>>,
...
pub enum Error {
    ...
    UISend(String),
    #[error("Cannot use root nrz.json at {0} with Watch Mode.")]
    NonStandardNrzJsonPath(String),
...
impl WatchClient {
    pub async fn new(
        ...
        if base.opts.repo_opts.root_nrz_json_path != base.repo_root.join_component(CONFIG_FILE) {
            return Err(Error::NonStandardNrzJsonPath(
                base.opts.repo_opts.root_nrz_json_path.to_string(),
            ));
        }
