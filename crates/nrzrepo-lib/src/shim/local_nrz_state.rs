...
/// Structure that holds information on an existing local nrz install
#[derive(Debug)]
pub struct LocalNrzState {
    bin_path: PathBuf,
    version: String,
}
...
impl LocalNrzState {
    ...
    fn generate_nested_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        Some(root_path.join_components(&["node_modules", "nrz", "node_modules"]))
    }
    ...
    fn generate_linked_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        // root_path/node_modules/nrz is a symlink. Canonicalize the symlink to what
        ...
        // well.
        let canonical_path =
            fs_canonicalize(root_path.as_path().join("node_modules").join("nrz")).ok()?;
    ...
    fn generate_unplugged_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        let platform_package_name = NrzState::platform_package_name();
    ...
    // We support six per-platform packages and one `nrz` package which handles
    ...
    // _worst_ outcome is that we run global `nrz`.
    //
    ...
    pub fn infer(root_path: &AbsoluteSystemPath) -> Option<Self> {
        let platform_package_name = NrzState::platform_package_name();
        let binary_name = NrzState::binary_name();
            ...
            .filter_map(|search_function| search_function(root_path))
        {
            ...
            match fs_canonicalize(&bin_path) {
                Ok(bin_path) => {
                    ...
                    debug!("Local nrz path: {}", bin_path.display());
                    debug!("Local nrz version: {}", &local_version);
                    return Some(Self {
                        bin_path,
                        version: local_version,
                    });
                }
                Err(_) => debug!("No local nrz binary found at: {}", bin_path),
            }
        }
...
pub fn nrz_version_has_shim(version: &str) -> bool {
...
mod test {
    ...
    fn test_skip_infer_version_constraint(version: &str, expected: bool) {
        assert_eq!(nrz_version_has_shim(version), expected);
    }
}
