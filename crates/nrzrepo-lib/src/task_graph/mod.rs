...
fn task_log_filename(task_name: &str) -> String {
    format!("nrz-{}.log", task_name.replace(':', "$colon$"))
}
...
mod test {
        ...
        assert_eq!(
            ...
            TaskOutputs {
                inclusions: vec![
                    ...
                    format!("{relative_prefix}.nrz/nrz-build.log"),
                ],
    ...
    fn test_escape_log_file() {
        ...
        let build_expected =
            AnchoredSystemPathBuf::from_raw([".nrz", "nrz-build.log"].join(MAIN_SEPARATOR_STR))
                .unwrap();
        ...
        let build_expected = AnchoredSystemPathBuf::from_raw(
            [".nrz", "nrz-build$colon$prod.log"].join(MAIN_SEPARATOR_STR),
        )
        .unwrap();
        assert_eq!(build_log, build_expected);
        ...
        let build_expected = AnchoredSystemPathBuf::from_raw(
            [".nrz", "nrz-build$colon$prod$colon$extra.log"].join(MAIN_SEPARATOR_STR),
        )
        .unwrap();
        assert_eq!(build_log, build_expected);
    }
