...
impl<'a> ExecContextFactory<'a> {
    ...
    // Add any env vars that `nrz` provides to the task environment
    fn populate_env(
        &self,
        ...
        execution_env.insert("NRZ_HASH".to_owned(), task_hash.to_owned());

        // Allow downstream tools to detect if the task is being ran with TUI
        if self.visitor.run_opts.ui_mode.use_tui() {
            execution_env.insert("NRZ_IS_TUI".to_owned(), "true".to_owned());
        }
...
impl ExecContext {
    ...
    async fn execute_inner(
        ...
        let mut process = match self.manager.spawn(cmd, Duration::from_millis(500)) {
            Some(Ok(child)) => child,
            // Nrz was unable to spawn a process
            Some(Err(e)) => {
        ...
        match exit_status {
            ...
            // The child was killed by nrz indicating a shutdown
