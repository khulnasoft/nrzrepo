...
use nrzrepo_repository::package_graph::{PackageGraph, PackageName, ROOT_PKG_NAME};
...
pub enum Error {
    ...
    RecursiveNrz {
        task_name: String,
        command: String,
        #[label("This script calls `nrz`, which calls the script, which calls `nrz`...")]
            ...
            match self.dry {
                ...
                false => {
                    ...
                    let Some(mut exec_context) = factory.exec_context(
                        ...
                        // TODO(gsoltis): if/when we fix https://github.com/khulnasoft/nrzrepo/issues/937
...
fn nrz_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?:^|\s)nrz(?:$|\s)").unwrap())
}
