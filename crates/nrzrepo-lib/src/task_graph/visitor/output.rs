...
impl<W: Write> TaskCacheOutput<W> {
    pub fn task_writer(&mut self) -> Either<nrzrepo_ui::PrefixedWriter<&mut W>, TaskSender> {
        match self {
...
impl<W: Write> CacheOutput for TaskCacheOutput<W> {
    ...
    fn replay_logs(&mut self, log_file: &AbsoluteSystemPath) -> Result<(), nrzrepo_ui::Error> {
        match self {
            TaskCacheOutput::Direct(direct) => {
                let writer = direct.output_prefixed_writer();
                nrzrepo_ui::replay_logs(writer, log_file)
            }
            TaskCacheOutput::UI(task) => nrzrepo_ui::replay_logs(task, log_file),
        }
    }
}
