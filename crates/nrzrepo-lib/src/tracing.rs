...
/// A basic logger that logs to stderr using the NrzFormatter.
/// The first generic parameter refers to the previous layer, which
/// is in this case the default layer (`Registry`).
type StdErrLog = fmt::Layer<Registry, DefaultFields, NrzFormatter, StdErrWrapper>;
...
/// - with evel `TRACE` or higher for the `nrzrepo` target
...
impl NrzSubscriber {
    ...
    ///   `NRZ_LOG_VERBOSITY` global setting, but not per-module settings.
    ///
    /// `NrzSubscriber` has optional loggers that can be enabled later:
    ...
    pub fn new_with_verbosity(verbosity: usize, color_config: &ColorConfig) -> Self {
        ...
        let env_filter = |level: LevelFilter| {
            let filter = EnvFilter::builder()
                ...
                .with_env_var("NRZ_LOG_VERBOSITY")
                .from_env_lossy()
...
impl<S, N> FormatEvent<S, N> for NrzFormatter
where
