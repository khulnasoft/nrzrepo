...
pub enum Error {
    ...
    #[error("Nrzrepo cannot serialize Bun lockfiles.")]
    NotImplemented,
}
...
impl Lockfile for BunLockfile {
    ...
    fn resolve_package(
        ...
        if let Some((_key, entry)) = self.package_entry(&workspace_key) {
            ...
            // faulty cache miss if used by nrz to calculate a hash.
    ...
    fn nrz_version(&self) -> Option<String> {
        let (_, entry) = self.package_entry("nrz")?;
...
mod test {
    ...
    #[test_case("", "nrz", "^2.3.3", "nrz@2.3.3" ; "root")]
    ...
    .as_slice();
    const NRZ_GEN_CHALK_DEPS: &[&str] = [
        "@nrz/gen/chalk/ansi-styles",
        "@nrz/gen/chalk/escape-string-regexp",
        "@nrz/gen/chalk/supports-color",
    ]
    .as_slice();
    const CHALK_DEPS: &[&str] = ["ansi-styles", "supports-color"].as_slice();

    #[test_case("@nrz/gen", NRZ_GEN_DEPS)]
    #[test_case("@nrz/gen/chalk", NRZ_GEN_CHALK_DEPS)]
