...
impl Lockfile for NpmLockfile {
    ...
    fn nrz_version(&self) -> Option<String> {
        let nrz_entry = self.packages.get("node_modules/nrz")?;
        nrz_entry.version.clone()
    }
...
mod test {
    ...
    fn test_all_dependencies() -> Result<(), Error> {
        ...
        let tests = [
            ...
            (
                "node_modules/nrz",
                vec![
                    "node_modules/nrz-darwin-64",
                    "node_modules/nrz-darwin-arm64",
                    "node_modules/nrz-linux-64",
                    "node_modules/nrz-linux-arm64",
                    "node_modules/nrz-windows-64",
                    "node_modules/nrz-windows-arm64",
                ],
            ),
            (
    ...
    fn test_workspace_peer_dependencies() -> Result<(), Error> {
        ...
        let closures = crate::all_transitive_closures(
            ...
            vec![
                (
                    "packages/a".into(),
                    vec![("eslint-plugin-nrz".into(), "^1.9.3".into())]
                        .into_iter()
                        .collect(),
                ),
        ...
        assert!(closures.get("packages/a").unwrap().contains(&Package {
            key: "node_modules/eslint-plugin-nrz".into(),
            version: "1.9.3".into()
        }));
    ...
    #[test]
    fn test_nrz_version() -> Result<(), Error> {
        let lockfile = NpmLockfile::load(include_bytes!("../fixtures/npm-lock.json"))?;
        assert_eq!(lockfile.nrz_version().as_deref(), Some("1.5.5"));
        Ok(())
    }
}
