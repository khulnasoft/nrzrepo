...
impl crate::Lockfile for PnpmLockfile {
    ...
    fn patches(&self) -> Result<Vec<RelativeUnixPathBuf>, crate::Error> {
        let mut patches = self
            ...
            .collect::<Result<Vec<_>, nrzpath::PathError>>()?;
        patches.sort();
        Ok(patches)
    }
    ...
    fn nrz_version(&self) -> Option<String> {
        let nrz_version = self
            .importers
            .values()
            // Look through all of the workspace packages for a nrz dependency
            // grab the first one we find.
            .find_map(|project| project.dependencies.nrz_version())?;
        Some(nrz_version.to_owned())
    }
...
impl DependencyInfo {
    ...
    fn nrz_version(&self) -> Option<&str> {
        let (_specifier, version) = self.find_resolution("nrz")?;
        Some(version)
    }
}
...
mod tests {
    ...
    const PNPM6_NRZ: &[u8] = include_bytes!("../../fixtures/pnpm6nrz.yaml").as_slice();
    const PNPM8_NRZ: &[u8] = include_bytes!("../../fixtures/pnpm8nrz.yaml").as_slice();
...
";
    ...
    #[test_case(PNPM6_NRZ, Some("2.0.3") ; "v6")]
    #[test_case(PNPM8_NRZ, Some("2.0.3") ; "v8")]
    #[test_case(PNPM_V9, Some("1.13.3-canary.1") ; "v9")]
    fn test_nrz_version(lockfile: &[u8], expected: Option<&str>) {
        let lockfile = PnpmLockfile::from_bytes(lockfile).unwrap();
        assert_eq!(lockfile.nrz_version().as_deref(), expected);
    }
}
