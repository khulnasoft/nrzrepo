...
impl Lockfile for Yarn1Lockfile {
    ...
    fn nrz_version(&self) -> Option<String> {
        // Yarn lockfiles can have multiple descriptors as a key e.g. nrz@latest,
        // nrz@1.2.3 We just check if the first descriptor is for nrz and
        // return that. Using multiple versions of nrz in a single project is
        // not supported.
        let key = self.inner.keys().find(|key| key.starts_with("nrz@"))?;
        let entry = self.inner.get(key)?;
...
mod test {
    ...
    fn test_nrz_version(lockfile: &str, expected: &str) {
        let lockfile = Yarn1Lockfile::from_str(lockfile).unwrap();
        assert_eq!(lockfile.nrz_version().as_deref(), Some(expected));
    }
}
