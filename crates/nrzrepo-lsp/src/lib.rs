...
//! as well as the architecture documentation in `packages/nrz-vsc`.
...
pub struct Backend {
    ...
    // this is only used for nrz optimize
...
impl LanguageServer for Backend {
            ...
            capabilities: ServerCapabilities {
                ...
                execute_command_provider: Some(ExecuteCommandOptions {
                    commands: vec![
                        ...
                        // "nrz.daemon.start".to_string(),
                        // "nrz.daemon.status".to_string(),
                        // "nrz.daemon.stop".to_string(),
                    ],
                    work_done_progress_options: Default::default(),
                }),
    ...
    /// Add code lens items for running a particular task in the nrz.json
        ...
        let workspaces = packages.map(|p| {
            chain(
                ...
                iter::once(WorkspaceData {
                    ...
                    nrz_json: root_nrz_json.exists().then_some(root_nrz_json),
                }),
            )
        });
...
fn report_invalid_packages_and_tasks(
    ...
    let (package, task) = package_task
        .value
        .split_once('#') // nrz packages may not have # in them
        .map(|(p, t)| (Some(p), t))
    ...
    match (tasks.get(task), package) {
        ...
        (_, Some(package)) if !packages.contains(&package) => {
            diagnostics.push(Diagnostic {
                ...
                range,
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String("nrz:no-such-package".to_string())),
                ..Default::default()
            });
        }
                ...
                .contains(&package) =>
        {
            diagnostics.push(Diagnostic {
                ...
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String(
                    "nrz:no-such-task-in-package".to_string(),
                )),
                ..Default::default()
            });
        }
        ...
        (None, None) => {
            diagnostics.push(Diagnostic {
                ...
                range,
                severity: Some(DiagnosticSeverity::ERROR),
                code: Some(NumberOrString::String("nrz:no-such-task".to_string())),
                ..Default::default()
            });
        }
