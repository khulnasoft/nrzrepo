...
const DEFAULT_GLOBAL_DEPS: [&str; 2] = ["package.json", "nrz.json"];
...
pub enum LockfileChange {
    Empty,
    ChangedPackages(HashSet<nrzrepo_lockfiles::Package>),
}
...
pub enum PackageInclusionReason {
    ...
    LockfileChanged {
        removed: Vec<nrzrepo_lockfiles::Package>,
        added: Vec<nrzrepo_lockfiles::Package>,
    },
...
pub enum AllPackageChangeReason {
    ...
    /// A file like `package.json` or `nrz.json` changed
...
impl<'a, PD: PackageChangeMapper> ChangeMapper<'a, PD> {
    ...
    pub fn lockfile_changed(
        ...
        let lockfile_path_relative = nrz_root
...
pub enum ChangeMapError {
    ...
    Lockfile(nrzrepo_lockfiles::Error),
}
...
mod test {
    ...
    fn test_lockfile_changed(
        ...
        let nrz_root = nrzpath::AbsoluteSystemPathBuf::new(nrz_root).unwrap();
        let lockfile_path = nrzpath::AbsoluteSystemPathBuf::new(lockfile_path).unwrap();
        let changed_files = changed_files
            .iter()
            .map(|s| nrzpath::AnchoredSystemPathBuf::from_raw(s).unwrap())
            .collect();
