...
pub struct PackageInfo {
    ...
    pub transitive_dependencies: Option<HashSet<nrzrepo_lockfiles::Package>>,
}
...
pub struct ExternalDependencyChange {
    ...
    pub added: Vec<nrzrepo_lockfiles::Package>,
    /// Dependencies that were removed from the package
    pub removed: Vec<nrzrepo_lockfiles::Package>,
}
...
impl PackageGraph {
    ...
    pub fn dependencies<'a>(&'a self, node: &PackageNode) -> HashSet<&'a PackageNode> {
        let mut dependencies = nrzrepo_graph_utils::transitive_closure(
            &self.graph,
    ...
    pub fn ancestors(&self, node: &PackageNode) -> HashSet<&PackageNode> {
        ...
        let mut dependents = if self.root_internal_dependencies().contains(node) {
            ...
            nrzrepo_graph_utils::transitive_closure(
                &self.graph,
                self.node_lookup.get(node).cloned(),
    ...
    pub fn transitive_external_dependencies<'a, I: IntoIterator<Item = &'a PackageName>>(
        &self,
        packages: I,
    ) -> HashSet<&nrzrepo_lockfiles::Package> {
        packages
            .into_iter()
    ...
    pub fn changed_packages_from_lockfile(
        ...
        let closures = nrzrepo_lockfiles::all_transitive_closures(previous, external_deps, true)?;
...
mod test {
    ...
    #[derive(Debug)]
    struct MockLockfile {}
    impl nrzrepo_lockfiles::Lockfile for MockLockfile {
        fn resolve_package(
            &self,
            _workspace_path: &str,
            name: &str,
            _version: &str,
        ) -> std::result::Result<Option<nrzrepo_lockfiles::Package>, nrzrepo_lockfiles::Error>
        {
            Ok(match name {
                "a" => Some(nrzrepo_lockfiles::Package::new("key:a", "1")),
                "b" => Some(nrzrepo_lockfiles::Package::new("key:b", "1")),
                "c" => Some(nrzrepo_lockfiles::Package::new("key:c", "1")),
                _ => None,
            })
        }
        ...
        fn all_dependencies(
            &self,
            key: &str,
        ) -> std::result::Result<Option<HashMap<String, String>>, nrzrepo_lockfiles::Error>
        {
            match key {
                "key:a" => Ok(Some(
                    [("c", "1")]
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> std::result::Result<Box<dyn Lockfile>, nrzrepo_lockfiles::Error> {
            unreachable!("lockfile pruning not necessary for package graph construction")
        }

        fn encode(&self) -> std::result::Result<Vec<u8>, nrzrepo_lockfiles::Error> {
    ...
    async fn test_lockfile_traversal() {
        ...
        let bar_deps = pkg_graph
            ...
            .as_ref()
            .unwrap();
        let a = nrzrepo_lockfiles::Package::new("key:a", "1");
        let b = nrzrepo_lockfiles::Package::new("key:b", "1");
        let c = nrzrepo_lockfiles::Package::new("key:c", "1");
