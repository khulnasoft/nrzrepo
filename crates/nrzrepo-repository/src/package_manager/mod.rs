...
impl Display for NoPackageManager {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        or run `npx @nrz/codemod add-package-manager` in the root of your monorepo.")
    }
}
...
impl PackageManager {
    ...
    pub fn parse_lockfile(
        ...
        Ok(match self {
            PackageManager::Npm => Box::new(nrzrepo_lockfiles::NpmLockfile::load(contents)?),
            PackageManager::Pnpm | PackageManager::Pnpm6 | PackageManager::Pnpm9 => {
                Box::new(nrzrepo_lockfiles::PnpmLockfile::from_bytes(contents)?)
            }
            PackageManager::Yarn => {
    ...
    pub fn lockfile_path(&self, nrz_root: &AbsoluteSystemPath) -> AbsoluteSystemPathBuf {
        nrz_root.join_component(self.lockfile_name())
    }
...
mod tests {
    ...
    fn repo_root() -> AbsoluteSystemPathBuf {
        ...
        panic!("Couldn't find Nrzrepo root from {}", cwd);
    }
