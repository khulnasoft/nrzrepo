...
impl SCM {
    ...
    pub fn changed_files(
        ...
        match self {
            ...
            Self::Manual => Err(Error::GitRequired(nrz_root.to_owned())),
        }
    }
...
impl Git {
    ...
    fn changed_files(
        ...
        let nrz_root_relative_to_git_root = self.root.anchor(nrz_root)?;
        let pathspec = nrz_root_relative_to_git_root.as_str();
        ...
        self.add_files_from_stdout(&mut files, nrz_root, output);
        ...
        if include_uncommitted {
            ...
            self.add_files_from_stdout(&mut files, nrz_root, ls_files_output);
            // Include any files that have been staged, but not committed
            let diff_output =
                self.execute_git_command(&["diff", "--name-only", "--cached"], pathspec)?;
            self.add_files_from_stdout(&mut files, nrz_root, diff_output);
        }
    ...
    fn add_files_from_stdout(
        ...
        for line in stdout.lines() {
            ...
            let anchored_to_nrz_root_file_path = self
                .reanchor_path_from_git_root_to_nrz_root(nrz_root, path)
                .unwrap();
            files.insert(anchored_to_nrz_root_file_path);
        }
    }

    fn reanchor_path_from_git_root_to_nrz_root(
        &self,
        nrz_root: &AbsoluteSystemPath,
        path: &RelativeUnixPath,
    ) -> Result<AnchoredSystemPathBuf, Error> {
        let absolute_file_path = self.root.join_unix_path(path);
        let anchored_to_nrz_root_file_path = nrz_root.anchor(&absolute_file_path)?;
        Ok(anchored_to_nrz_root_file_path)
    }
...
mod tests {
    ...
    fn changed_files(
        ...
        let nrz_root = AbsoluteSystemPathBuf::try_from(nrz_root.as_path())?;
    ...
    fn test_shallow_clone() -> Result<(), Error> {
        ...
        let output = Command::new(git_binary)
            .args([
                ...
                "--depth",
                "2",
                "https://github.com/khulnasoft/app-playground.git",
    ...
    fn test_deleted_files() -> Result<(), Error> {
        ...
        let nrzrepo_root = repo_root.path().to_owned();
    ...
    fn test_changed_files() -> Result<(), Error> {
        ...
        // Test that `nrz_root` filters out files not in the specified directory
        let files = changed_files(
            repo_root.path().to_path_buf(),
    ...
    // Tests that we can use a subdir as the nrz_root path
    // (occurs when the monorepo is nested inside a subdirectory of git repository)
    #[test]
    fn test_changed_files_with_subdir_as_nrz_root() -> Result<(), Error> {
        let (repo_root, repo) = setup_repository(None)?;
    ...
    fn test_error_cases() -> Result<(), Error> {
        ...
        let nrz_root = tempfile::tempdir()?;
        let nrz_root_is_not_subdir_of_git_root = changed_files(
            repo_root.path().to_path_buf(),
            nrz_root.path().to_path_buf(),
            Some("HEAD"),
            None,
            true,
        );
        ...
        assert_matches!(
            nrz_root_is_not_subdir_of_git_root,
