...
//! Nrzrepo's library for interacting with source control management (SCM).
...
pub enum Error {
    ...
    #[error("Unable to resolve base branch. Please set with `NRZ_SCM_BASE`.")]
    UnableToResolveRef,
}
...
fn find_git_root(nrz_root: &AbsoluteSystemPath) -> Result<AbsoluteSystemPathBuf, Error> {
    let rev_parse = Command::new("git")
        .args(["rev-parse", "--show-cdup"])
        .current_dir(nrz_root)
        .output()?;
    ...
    if let Some(line) = lines.next() {
        ...
        Ok(nrz_root.join_unix_path(tail))
    } else {
        let stderr = String::from_utf8_lossy(&rev_parse.stderr);
...
mod tests {
    ...
    fn test_symlinked_git_root() {
        ...
        let nrz_root = link.join_component("inside");
        let result = find_git_root(&nrz_root).unwrap();
