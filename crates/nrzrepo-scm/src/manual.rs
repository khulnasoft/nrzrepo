...
pub(crate) fn get_package_file_hashes_without_git<S: AsRef<str>>(
    ...
    let full_package_path = nrz_root.resolve(package_path);
    let mut hashes = GitHashes::new();
...
mod tests {
    ...
    fn test_hash_files(files: &[&str], allow_missing: bool, want_err: bool) {
        let (_tmp, nrz_root) = tmp_dir();
        let test_file = nrz_root.join_component("existing-file.txt");
        ...
        match hash_files(&nrz_root, files, allow_missing) {
    ...
    fn test_hash_symlink() {
        let (_tmp, nrz_root) = tmp_dir();
        let from_to_file = nrz_root.join_component("symlink-from-to-file");
        let from_to_dir = nrz_root.join_component("symlink-from-to-dir");
        let broken = nrz_root.join_component("symlink-broken");

        let to_file = nrz_root.join_component("the-file-target");
        to_file.create_with_contents("contents").unwrap();

        let to_dir = nrz_root.join_component("the-dir-target");
    ...
    fn test_get_package_file_hashes_from_processing_gitignore() {
        ...
        let mut file_hash: Vec<(&str, &str, Option<&str>)> = vec![
            ("nrz.json", "nrz.json-file-contents", None),
            ...
            (
                "child-dir/libA/nrz.json",
                "lib-nrz.json-content",
        ...
        let root_ignore_file = nrz_root.join_component(".gitignore");
        root_ignore_file
            .create_with_contents(root_ignore_contents)
            .unwrap();
        let pkg_ignore_file = nrz_root.resolve(&pkg_path).join_component(".gitignore");
        ...
        for (raw_unix_path, contents, expected_hash) in file_hash.iter() {
            ...
            let file_path = nrz_root.join_unix_path(unix_path);
        ...
        for (raw_unix_path, contents, expected_hash) in file_hash.iter() {
            ...
            if let Some(hash) = expected_hash {
                ...
                if (unix_pkg_file_path.ends_with("file")
                    ...
                    || unix_pkg_file_path.ends_with("nrz.json"))
