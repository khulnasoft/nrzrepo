...
pub const INPUT_INCLUDE_DEFAULT_FILES: &str = "$NRZ_DEFAULT$";
...
impl SCM {
    pub fn get_hashes_for_files(
        ...
        if allow_missing {
            self.hash_existing_of(nrz_root, files.iter())
        } else {
            self.hash_files(nrz_root, files.iter())
        }
    }

    #[tracing::instrument(skip(self, nrz_root, package_path, inputs))]
    pub fn get_package_file_hashes<S: AsRef<str>>(
        ...
        // If the inputs contain "$NRZ_DEFAULT$", we need to include the "default"
        // file hashes as well. NOTE: we intentionally don't remove
        // "$NRZ_DEFAULT$" from the inputs if it exists in the off chance that
        // the user has a file named "$NRZ_DEFAULT$" in their package (pls
        // no).
    ...
    pub fn hash_files(
        ...
        match self {
            SCM::Manual => crate::manual::hash_files(nrz_root, files, false),
            SCM::Git(git) => git.hash_files(nrz_root, files),
        }
    }
    ...
    pub fn hash_existing_of(
        ...
        crate::manual::hash_files(nrz_root, files, true)
    }
}
...
impl Git {
    fn get_package_file_hashes<S: AsRef<str>>(
        ...
        // no inputs, and no $NRZ_DEFAULT$
        if inputs.is_empty() {
            return self.get_package_file_hashes_from_index(nrz_root, package_path);
        }
        ...
        self.get_package_file_hashes_from_inputs_and_index(nrz_root, package_path, inputs)
    }

    #[tracing::instrument(skip(self, nrz_root))]
    fn get_package_file_hashes_from_index(
        &self,
        nrz_root: &AbsoluteSystemPath,
        package_path: &AnchoredSystemPath,
    ) -> Result<GitHashes, Error> {
        let full_pkg_path = nrz_root.resolve(package_path);
    ...
    #[tracing::instrument(skip(self, nrz_root, inputs))]
    fn get_package_file_hashes_from_inputs<S: AsRef<str>>(
        ...
        if include_configs {
            // Add in package.json and nrz.json to input patterns. Both file paths are
            // relative to pkgPath
            //
    ...
    fn get_package_file_hashes_from_inputs_and_index<S: AsRef<str>>(
        ...
        let default_file_hashes =
            self.get_package_file_hashes_from_index(nrz_root, package_path)?;
        ...
        let manual_includes_hashes =
            self.get_package_file_hashes_from_inputs(nrz_root, package_path, &includes, true)?;

        // only run the excludes search if there are excludes
        let manual_excludes_hashes = if !excludes.is_empty() {
            self.get_package_file_hashes_from_inputs(nrz_root, package_path, &excludes, false)?
        } else {
            GitHashes::new()
        };
...
mod tests {
    ...
    fn test_get_package_deps() -> Result<(), Error> {
        ...
        // create a root nrz.json
        let root_nrz_json_path = repo_root.join_component("nrz.json");
        root_nrz_json_path.create_with_contents("{}")?;
        ...
        // create a package nrz.json
        let nrz_json_path = my_pkg_dir.join_component("nrz.json");
        nrz_json_path.create_with_contents("{}")?;
        ...
        // create a file that matches the $NRZ_DEFAULT$ token to test the edge case
        let token_file_path = my_pkg_dir.join_components(&["$NRZ_DEFAULT$"]);
        ...
        let all_expected = to_hash_map(&[
            ...
            ("nrz.json", "9e26dfeeb6e641a33dae4961196235bdb965b21b"),
            (
                "dir/nested-file",
        ...
        all_expected.insert(
            RelativeUnixPathBuf::new("$NRZ_DEFAULT$").unwrap(),
        ...
        let input_tests: &[(&[&str], &[&str])] = &[
            (
                &["uncommitted-file"],
                &["package.json", "nrz.json", "uncommitted-file"],
            ),
            (
                &["**/*-file"],
                &[
            ...
            (
                &["$NRZ_DEFAULT$", "!committed-file", "dir/ignored-file"],
                &[
                    "uncommitted-file",
                    "package.json",
                    "nrz.json",
            ...
            (
                &["!committed-file", "$NRZ_DEFAULT$", "dir/ignored-file"],
                &[
                    "uncommitted-file",
                    "package.json",
                    "nrz.json",
