...
///
/// https://github.com/khulnasoft/nrzrepo/blob/main/packages/nrz-telemetry/src/config.ts
use std::env;
...
static DISABLED_ENV_VAR: &str = "NRZ_TELEMETRY_DISABLED";
static DISABLED_MESSAGE_ENV_VAR: &str = "NRZ_TELEMETRY_MESSAGE_DISABLED";
...
impl TelemetryConfig {
    ...
    pub fn new(config_path: AbsoluteSystemPathBuf) -> Result<TelemetryConfig, ConfigError> {
        ...
        // isolated from other nrzrepo config, try to remove the entire config
        ...
        if !self.has_seen_alert() && self.is_enabled() && Self::is_telemetry_warning_enabled() {
            eprintln!(
                ...
                color!(
                    ...
                    GREY,
                    "{}",
                    "This information is used to shape the Nrzrepo roadmap and prioritize \
                     features."
                ),
                color!(
                    color_config,
    ...
    pub fn is_enabled(&self) -> bool {
        ...
        let nrz_telemetry_disabled = env::var(DISABLED_ENV_VAR).unwrap_or("0".to_string());

        if do_not_track == "1"
            || do_not_track == "true"
            || nrz_telemetry_disabled == "1"
            || nrz_telemetry_disabled == "true"
        {
            return false;
        }
...
fn default_config_path() -> Result<AbsoluteSystemPathBuf, ConfigError> {
    ...
    // stored as a sibling to the nrz global config
    Ok(config_dir.join_components(&["nrzrepo", "telemetry.json"]))
}
