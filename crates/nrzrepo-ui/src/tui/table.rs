    ...
    fn finished_rows(&self) -> impl Iterator<Item = Row> + '_ {
        self.tasks_by_type.finished.iter().map(move |task| {
            ...
            Row::new(vec![
                name,
                match task.result() {
                    // matches Next.js (and many other CLI tools) https://github.com/khulnasoft/next.js/blob/1a04d94aaec943d3cce93487fea3b8c8f8898f31/packages/next/src/build/output/log.ts
                    TaskResult::Success => {
