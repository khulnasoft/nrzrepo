...
//! Nrzrepo's update notifier. Indicates to the user that there is a new
//! version of `nrz` available.
...
const NOTIFIER_DISABLE_VARS: [&str; 2] = ["NO_UPDATE_NOTIFIER", "NRZ_NO_UPDATE_NOTIFIER"];
...
pub fn display_update_check(
    ...
    if let Ok(Some(version)) = version {
        ...
        let update_cmd = style("npx @nrz/codemod@latest update").cyan().bold();
