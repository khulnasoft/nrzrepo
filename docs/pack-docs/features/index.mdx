---
title: Features
description: Learn about Nrzpack's supported features
---
...
**Nrzpack is in beta**, so very few of these decisions are set in stone. In its current state, **Nrzpack cannot yet be configured** - so plugins are not available yet.

Let's discuss which features are available out-of-the-box, in Nrzpack's default configuration. We'll also touch on features which will be configurable via plugins.
