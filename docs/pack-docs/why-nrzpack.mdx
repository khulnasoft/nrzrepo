---
title: Why Nrzpack?
description: Learn why we think Nrzpack is the future of bundling for the web.
---

When we set out to create Nrzpack, we wanted to solve a problem. We had been working on speed improvements for Next.js. We migrated away from several JS-based tools. Babel, gone. Terser, gone. Our next target was another JS-based tool, webpack.
...
We wanted to remove this maintenance burden from Next.js and any framework that chooses to use Nrzpack. We can also create a cleaner and more stable implementation by designing a single unified graph that can be used to generate bundles for multiple environments.
...
That’s why we decided that, like webpack, we wanted Nrzpack to bundle the code in the development server. Nrzpack can do it much faster, especially for larger applications, because it is written in Rust and skips optimization work that is only necessary for production.
...
We decided to create a reusable Nrz build engine, similar to Parcel's request manager and rustc's query system, for distributed and incremental behavior. The Nrz engine works like a scheduler for function calls, allowing calls to functions to be parallelized across all available cores.

The Nrz engine also caches the result of all the functions it schedules, meaning it never needs to do the same work twice. Put simply, **it does the minimum work at maximum speed**.
...
Nrzpack’s development mode builds a minimal graph of your app’s imports and exports based on received requests and only bundles the minimal code necessary. Learn more in the [incremental computation docs](/pack/docs/incremental-computation).
...
- Use incremental computation. The Nrz engine brings this into the core of Nrzpack’s architecture - maximizing speed and minimizing work done.
- Optimize our dev server’s startup time. For that, we build a lazy asset graph to compute only the assets requested.

That’s why we chose to build Nrzpack.
