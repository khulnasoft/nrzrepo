...
description: Nrzrepo builds a Task Graph based on your configuration and repository structure.
---
...
The Package Graph is the structure of your monorepo created by your package manager. When you install [Internal Packages](/repo/docs/core-concepts/internal-packages) into each other, Nrzrepo will automatically identify those dependency relationships to build a foundational understanding of your Workspace.

This sets the groundwork for the Task Graph, where you'll define how **tasks** relate to each other.

## Task Graph

In `nrz.json`, you express how tasks relate to each other. You can think of these relationships as
...
Nrzrepo uses a data structure called a [directed acyclic graph (DAG)](https://en.wikipedia.org/wiki/Directed_acyclic_graph) to
...
Nrzrepo will build a task graph like this:
...
the `ui` package does not. You also have a `nrz.json` that configures the
`build` task the same way as above with `"dependsOn": ["^build"]`. When you run
`nrz run build`, what would you expect to happen?

Nrzrepo will build this Task Graph:
...
Nrzrepo calls the `ui` package a Transit Node in this scenario, because it
doesn't have its own `build` script. Since it doesn't have a `build` script,
Nrzrepo won't execute anything for it, but it's still part of the graph for
...
Nrzrepo's mental model is that all nodes in the Task Graph are the same. In other words,
...
`test` tasks, but only one app has a `build` task. Nrzrepo's mental model
says that when you run `nrz run build`, even if an app doesn't implement `build`
