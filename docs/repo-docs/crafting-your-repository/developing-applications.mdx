...
Most development tasks are long-running tasks that watch for changes to your code. Nrzrepo enhances this experience with a powerful terminal UI and other capabilities like:
...
Defining a development task in `nrz.json` tells Nrzrepo that you'll be running a long-lived task. This is useful for things like running a development server, running tests, or building your application.

To register a `dev` task, add it to your `nrz.json` with two properties:
...
- **"cache": false**: Tells Nrzrepo to not attempt to cache the results of the task. Since this is a development task, you're likely to be making frequent changes to your code, so caching the results is not useful.
- **"persistent": true**: Tells Nrzrepo to keep the task running until you stop it. This key serves as a signal for your terminal UI to treat the task as long-running and interactive. Additionally, it prevents you from accidentally depending on a task that will not exit.
...
Nrzrepo's terminal UI enables a number of features that create a highly interactive experience around your tasks.
...
`nrz watch` adds a dependency-aware watcher to any tool. Changes to source code will follow [the Task Graph](/repo/docs/core-concepts/package-and-task-graph#task-graph) that you've described in `nrz.json`, just like all your other tasks.

For example, using a package structure like [`create-nrz`](/repo/docs/reference/create-nrz) with the following tasks and scripts:

<Tabs items={["nrz.json", "packages/ui", "apps/web"]}>
<Tab value="nrz.json">
...
When you run `nrz watch dev lint`, you'll see the `lint` scripts are re-run whenever you make source code changes, despite ESLint not having a built-in watcher. `nrz watch` is also aware of internal dependencies, so a code change in `@repo/ui` will re-run the task in both `@repo/ui` and `web`.
...
For more information, [visit the `nrz watch` reference](/repo/docs/reference/watch).

## Limitations

### Teardown tasks

In some cases, you may want to run a script when the `dev` task is stopped. Nrzrepo is unable to run those teardown scripts when exiting because `nrz` exits when your `dev` tasks exit.

Instead, create a `nrz dev:teardown` script that you run separately after you've exited your primary `nrz dev` task.

## Next steps

Once you have a version of your application that you'd like to deploy, it's time to learn how to configure environment variables in Nrzrepo.
