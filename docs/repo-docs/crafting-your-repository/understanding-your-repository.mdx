...
description: Learn how to understand your repository structure using Nrzrepo.
---

Nrzrepo includes tools for understanding your repository structure, that can help you use and optimize your codebase.

## `nrz ls`

To list your packages, you can run `nrz ls`. This will show the packages in your repository and where they're located.
...
To determine which tasks can be run in your monorepo, simply call `nrz run` without any tasks. You will get a list of
...
If you wish to dig into your repository structure, since `2.2.0`, Nrz provides a GraphQL interface into your repository
via `nrz query`. You can execute queries such as finding all packages that have a `test` task:

```bash title="Terminal"
> nrz query "query { packages(filter: { has: { field: TASK_NAME, value: \"build\"}}) { items { name } } }"
{
  "data": {
    "packages": {
      "items": [
...
```bash title="Terminal"
> nrz query "query { packages(filter: { greaterThan: { field: DIRECT_DEPENDENT_COUNT, value: 10 } }) { items { name } } }"
{
  "data": {
    "packages": {
      "items": [
...
With `nrz query`, you can find all the packages and the reason why they were invalidated:

```bash title="Terminal"
> nrz query "query { affectedPackages(base: \"HEAD^\", head: \"HEAD\") { items { reason {  __typename } } } }"
{
  "data": {
    "affectedPackages": {
      "items": [
