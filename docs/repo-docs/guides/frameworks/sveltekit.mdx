...
To get started with SvelteKit in a Nrzrepo quickly, use [the `with-svelte` example](https://github.com/khulnasoft/nrzrepo/tree/main/examples/with-svelte):

<PackageManagerTabs>
<Tab>

```bash title="Terminal"
npx create-nrz@latest -e with-svelte
```
...
```bash title="Terminal"
yarn dlx create-nrz@latest -e with-svelte
```
...
```bash title="Terminal"
pnpm dlx create-nrz@latest -e with-svelte
```
...
By default, the new application will use the tasks defined in the root `nrz.json`. If you'd like to configure tasks differently for the new application, use [Package Configurations](/repo/docs/reference/package-configurations).
