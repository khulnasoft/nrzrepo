...
To get started with Vite in a Nrzrepo quickly, use [the `with-vite` example](https://github.com/khulnasoft/nrzrepo/tree/main/examples/with-vite):
...
```bash title="Terminal"
yarn dlx create-nrz@latest -e with-vite
```
...
```bash title="Terminal"
pnpm dlx create-nrz@latest -e with-vite
```
