...
description: Learn how to generate code using Nrzrepo.
---
...
experience. With Nrzrepo's code generation, it's easy to generate new source code for packages, modules,
...
```sh title="Terminal"
nrz gen workspace --copy
```

Create a new workspace in your monorepo by copying from a remote package.

```sh title="Terminal"
nrz gen workspace --copy https://github.com/khulnasoft/nrzrepo/tree/main/examples/with-tailwind/packages/tailwind-config
```

<Callout>
  **Note**: When adding from a remote source, Nrzrepo is unable to verify that your repo has all of the required dependencies, and is using the correct package manager. In this case, some manual modifications may be required to get the new workspace working as expected within your repository.
...
Nrzrepo will automatically detect any generator configurations within your repo, and make them available to run from the command line.

<Callout type="info">
  While Nrzrepo Generators are built on top of Plop, they don't require `plop`
  to be installed as a dependency in your repo.
</Callout>

While Nrzrepo understands all Plop configuration options and features, it provides a few additional features to improve the experience of writing
generators within a repo configured with Nrzrepo.
...
To build and run a custom generator, run the following command from anywhere within your monorepo using Nrzrepo.
...
manually at `nrz/generators/config.ts` (or `config.js`) at the root of your repo - or within _any_ workspace.

<Callout type="info">
  If you are using TypeScript, you will need to install [the `@nrz/gen`
  package](https://github.com/khulnasoft/nrzrepo/tree/main/packages/nrz-gen) as
...
<Files>
  <Folder name="apps" defaultOpen>
    ...
    <Folder name="web" defaultOpen>
      <File name="package.json"  />
  <Folder name="nrz" green defaultOpen>
...
```ts title="nrz/generators/config.ts"
