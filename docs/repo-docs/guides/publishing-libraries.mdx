...
2. Add `dist` to the outputs of `build` in your `nrz.json`.
...
That way, when `tsup` is run the outputs can be [cached](/repo/docs/crafting-your-repository/caching) by Nrzrepo.
...
Before we can run `nrz run build`, there's one thing we need to consider. We've just added a [task dependency](/repo/docs/crafting-your-repository/running-tasks) into our monorepo. The `build` of `packages/math` needs to run **before** the `build` of `apps/web`.
...
Now, we can run `nrz run build`, and it'll automatically build our packages _before_ it builds our app.
...
If we've already set up [dev scripts](/repo/docs/crafting-your-repository/developing-applications#configuring-development-tasks) in our `nrz.json`, running `nrz run dev` will run our `packages/math` dev task in parallel with our `apps/web` dev task.
...
We recommend Changesets because it's intuitive to use, and - just like Nrzrepo - fits with the monorepo tools you're already used to.
...
Linking your publishing flow into Nrzrepo can make organizing your deploy a lot simpler and faster.
...
```json title="./package.json"
{
  "scripts": {
    ...
    // before publishing
    "publish-packages": "nrz run build lint test && changeset version && changeset publish"
  }
}
```
...
This means that when you run `publish-packages`, your monorepo gets built, linted, tested and published - and you benefit from all of Nrzrepo's speedups.
