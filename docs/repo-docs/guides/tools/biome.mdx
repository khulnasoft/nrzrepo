---
title: Biome
description: Learn how to use Biome in your Nrzrepo projects.
---
...
## Using Biome with Nrzrepo

Biome is a rare exception to most tools that are used with Nrzrepo because it is **so extraordinarily fast**. For this reason, we recommend using a [Root Task](/repo/docs/crafting-your-repository/configuring-tasks#registering-root-tasks) rather than creating separate scripts in each of your packages.
...
To create a [Root Task](/repo/docs/crafting-your-repository/configuring-tasks#registering-root-tasks), register the scripts to Nrzrepo:
...
You'll now be able to run these scripts using `nrz run format-and-lint` and `nrz run format-and-lint:fix`.
