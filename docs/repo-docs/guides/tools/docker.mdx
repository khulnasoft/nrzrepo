...
Docker is pretty smart about how it deploys your apps. Just like Nrzrepo, it tries to do as [little work as possible](https://bitjudo.com/blog/2014/03/13/building-efficient-dockerfiles-node-dot-js/).
...
The solution is to prune the inputs to the Dockerfile to only what is strictly necessary. Nrzrepo provides a simple solution - `nrz prune`.

```bash title="Terminal"
nrz prune api --docker
```
...
By default, `nrz prune` puts all relevant files inside `./out`. But to optimize caching with Docker, we ideally want to copy the files over in two stages.
...
Our detailed [`with-docker` example](https://github.com/khulnasoft/nrzrepo/tree/main/examples/with-docker) goes into depth on how to use `prune` to its full potential. Here's the Dockerfile, copied over for convenience.
...
# RUN yarn global add nrz@^2
RUN yarn global add nrz@^<your-major-version>
COPY . .

# Generate a partial monorepo with a pruned lockfile for a target workspace.
# Assuming "web" is the name entered in the project's package.json: { name: "web" }
RUN nrz prune web --docker
...
Assuming you are using a Dockerfile similar to the one above, we will bring in some environment variables from build arguments right before `nrz build`:

```docker title="./apps/api/Dockerfile"
ARG NRZ_TEAM
ENV NRZ_TEAM=$NRZ_TEAM

ARG NRZ_TOKEN
ENV NRZ_TOKEN=$NRZ_TOKEN

RUN yarn nrz run build
```

`nrz` will now be able to hit your Remote Cache. To see a Nrzrepo cache hit for a non-cached Docker build image, run a command like this one from your project root:
