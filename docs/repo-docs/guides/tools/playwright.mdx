---
title: Playwright
description: Learn how to use Playwright in a Nrzrepo.
---
...
This relationship can be expressed in `nrz.json` and the end-to-end suite's `package.json`.
...
Later on, when you want to run your end-to-end tests, use [the `--only` flag](/repo/docs/reference/run#--only) to run end-to-end tests without running the application's build first. As an example, your command may look like `nrz run e2e --filter=@repo/playwright-myapp --only`.
