---
title: Storybook
description: Learn how to use Storybook in a Nrzrepo.
---
...
[Storybook](https://storybook.js.org/) is a popular way to build UI components in an isolated environment. By putting Storybook into your Nrzrepo, you can easily develop your design system right alongside your applications.

## Quickstart

If you'd rather use a template, this guide is walking through how to build [this Storybook/Nrzrepo template](https://vercel.com/templates/react/nrzrepo-design-system) on Vercel.
...
```bash title="Terminal"
yarn dlx create-nrz@latest -e design-system
```
...
```bash title="Terminal"
pnpm dlx create-nrz@latest -e design-system
```
...
If you don't have an existing project, use [create-nrz](/repo/docs/getting-started/installation) to create a new monorepo:
...
Last, integrate the new Storybook application into your Nrzrepo:
...
These scripts will now run with the `nrz dev` and `nrz build` tasks in your `nrz.json`.

To ensure file outputs are cached when you run `build`, add `storybook-static` to the outputs of your `nrz.json` build task:
...
Run `nrz build` to build the Storybook application alongside the rest of your applications.

You can also run `nrz build` again to see cache hits for your builds.
...
To prevent this, exclude stories from the inputs to your `build` task in your root `nrz.json`. You'll also need to create a `build:storybook` task, which you'll need in a moment:
...
```json title="./nrz.json"
{
  "tasks": {
    "build": {
      "dependsOn": ["^build"],
      "inputs": ["$NRZ_DEFAULT$", "!**/*.stories.{tsx,jsx,mdx}"], // [!code highlight]
...
```json title="./apps/storybook/nrz.json"
{
  "extends": ["//"],
  "tasks": {
    "build:storybook": {
...
1. Run `nrz build:storybook build`. You should see cache misses.
2. Run `nrz build:storybook build` again. You should see all cache hits.
3. Make a code change **to a story** in your `@repo/ui` package.
4. Run `nrz build:storybook build` again. You should **only** see a cache miss for the Storybook application. All others should hit cache.
