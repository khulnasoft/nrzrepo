---
title: generate
description: API reference for the `nrz generate` command
---

import { Callout } from '#/components/callout';

Extend your Nrzrepo with new apps and packages.

```bash title="Terminal"
nrz generate
```

- [`nrz generate run [generator-name]`](#run-generator-name): Run custom generators defined in your repository.
- [`nrz generate workspace [options]`](#workspace): Create a new package in your repository by copying an existing one or from the start.
...
<Callout type="good-to-know">
  `nrz gen` is an alias for `nrz generate`. Additionally, `run` is the
  default command so `nrz gen` is equivalent to `nrz generate run`.
</Callout>
...
Default: `nrz/generators/config.js`

#### `--root <path>`

The root of your repository

Default: directory with root `nrz.json`
...
The root of your repository. Defaults to the directory of the root `nrz.json`.
