---
title: ls
description: API reference for the `nrz ls` command
---
...
- You can override `main` as the default base by setting `NRZ_SCM_BASE`.
- You can override `HEAD` as the default head by setting `NRZ_SCM_HEAD`.

```bash title="Terminal"
NRZ_SCM_BASE=development nrz ls --affected
```
