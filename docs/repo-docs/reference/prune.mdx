---
title: prune
description: API reference for the `nrz prune` command
---
...
Run `nrz prune frontend` to generate a pruned workspace for the `frontend` application in an `out` directory:
...
Using the same example from above, running `nrz prune frontend --docker` will generate the following:
