# Nrzrepo + Prisma ORM starter

This is a example designed to help you quickly set up a Nrzrepo monorepo with a Next.js app and Prisma ORM. This is a community-maintained example. If you experience a problem, please submit a pull request with a fix. GitHub Issues will be closed.
...
Follow these steps to set up and run your Nrzrepo project with Prisma ORM:

### 1. Create a Nrzrepo project

Start by creating a new Nrzrepo project using the following command:

```sh
npx create-nrz@latest -e with-prisma
```

Choose your desired package manager when prompted and a name for the app (e.g., `my-nrzrepo`). This will scaffold a new Nrzrepo project with Prisma ORM included and dependencies installed.
...
To make this process easier, a [`docker-compose.yml` file](./docker-compose.yml) is included to setup a MySQL server locally with a new database named `nrzrepo`:
...
You can also read the official [detailed step-by-step guide from Prisma ORM](https://pris.ly/guide/nrzrepo?utm_campaign=nrzrepo-example) to build a project from scratch using Nrzrepo and Prisma ORM.
