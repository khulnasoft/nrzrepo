...
Validate that we only run `my-app#build` with change not committed
  $ ${NRZ} run build --affected --log-order grouped
...
Do the same thing with the `query` command
  $ ${NRZ} query "query { affectedPackages { items { name reason { __typename } } } }"
...
Do the same thing with the `query` command
  $ ${NRZ} query "query { affectedPackages { items { name } } }"
   WARNING  query command is experimental and may change in the future
  {
    "data": {
      "affectedPackages": {
...
Override the SCM base to be HEAD, so nothing runs
  $ NRZ_SCM_BASE="HEAD" ${NRZ} run build --affected --log-order grouped
...
Do the same thing with the `ls` command
  $ NRZ_SCM_BASE="HEAD" ${NRZ} ls --affected
   WARNING  ls command is experimental and may change in the future
  0 no packages (npm)
...
Do the same thing with the `query` command
  $ ${NRZ} query "query { affectedPackages(base: \"HEAD\") { items { name } } }"
   WARNING  query command is experimental and may change in the future
  {
    "data": {
      "affectedPackages": {
...
Override the SCM head to be main, so nothing runs
  $ NRZ_SCM_HEAD="main" ${NRZ} run build --affected --log-order grouped
  \xe2\x80\xa2 Packages in scope:  (esc)
  \xe2\x80\xa2 Running build in 0 packages (esc)
  \xe2\x80\xa2 Remote caching disabled (esc)
...
Do the same thing with the `ls` command
  $ NRZ_SCM_HEAD="main" ${NRZ} ls --affected
   WARNING  ls command is experimental and may change in the future
  0 no packages (npm)
...
Do the same thing with the `query` command
  $ ${NRZ} query "query { affectedPackages(head: \"main\") { items { name } } }"
   WARNING  query command is experimental and may change in the future
  {
    "data": {
      "affectedPackages": {
...
Now try running `--affected` again, we should run all tasks
  $ ${NRZ} run build --affected --dry-run json | jq '.tasks | map(.taskId)| sort'
...
Use a filter with `affectedPackages`
  $ ${NRZ} query "query { affectedPackages(filter: { equal: { field: NAME, value: \"my-app\" } }) { items { name } } }"
   WARNING  query command is experimental and may change in the future
   WARNING  unable to detect git range, assuming all files have changed: Git error: fatal: no merge base found
