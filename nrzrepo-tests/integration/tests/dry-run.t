...
# https://stackoverflow.com/a/33297878/986415
  $ ${NRZ} run build --dry |  awk -v RS= '{print > ("tmp-" NR ".txt")}'
...
# Part 3 are Tasks to Run, and we have to validate each task separately
  ...
  my-app#build
    ...
    Log File                       = apps(\/|\\)my-app(\/|\\)\.nrz(\/|\\)nrz-build\.log\s* (re)
    Dependencies                   =\s* (re)
    Dependents                     =\s* (re)
  ...
  util#build
    ...
    Command                        = echo building\s* (re)
    Outputs                        =\s* (re)
    Log File                       = packages(\/|\\)util(\/|\\)\.nrz(\/|\\)nrz-build\.log\s* (re)
    Dependencies                   =\s* (re)
    Dependents                     =\s* (re)
...
# Run the task with NODE_ENV set and see it in summary. Use util package so it's just one package
  $ NODE_ENV=banana ${NRZ} run build --dry --filter=util | grep "Environment Variables"
  [1]
