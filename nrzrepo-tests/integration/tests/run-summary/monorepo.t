...
# Delete all run summaries
  ...
  $ ${NRZ} run build --summarize -- someargs > /dev/null # first run (should be cache miss)
...
# Extract some task-specific summaries from each
  ...
  [
    "apps(\/|\\\\)my-app(\/|\\\\).nrz(\/|\\\\)nrz-build.log" (re)
  ]
...
# Delete all run summaries
  ...
  $ ${NRZ} run build --summarize --no-daemon -- someargs > /dev/null # first run (should be cache miss)
...
# failures, it *should* be safe to remove.
  $ sleep 1
  $ ${NRZ} run build --summarize --no-daemon -- someargs > /dev/null # run again (expecting full nrz here)
