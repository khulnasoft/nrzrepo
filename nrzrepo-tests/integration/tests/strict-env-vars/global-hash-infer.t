...
There's no config to start, so the global hash does not change when flag is passed
  $ WITH_FLAG=$(${NRZ} build -vv --env-mode=infer 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
...
Add empty config for global pass through env var, global hash changes
  ...
  $ WITH_EMPTY_GLOBAL=$(${NRZ} build -vv --env-mode=infer 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
...
Add global pass through env var, global hash changes again, because we changed the value
  ...
  $ WITH_GLOBAL=$(${NRZ} build -vv --env-mode=infer 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
