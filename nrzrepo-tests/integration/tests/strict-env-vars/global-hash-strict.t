...
Hash changes, because we're using a new mode
  $ WITH_FLAG=$(${NRZ} build -vv --env-mode=strict 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
...
Hash does not change, because the mode is the same and we haven't added any new pass through vars
  ...
  $ WITH_EMPTY_GLOBAL=$(${NRZ} build -vv --env-mode=strict 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
...
Hash changes, because we have a new pass through value
  ...
  $ WITH_GLOBAL=$(${NRZ} build -vv --env-mode=strict 2>&1 | "$TESTDIR/../../../helpers/find_global_hash.sh")
