...
We can scope the run to specific packages
  $ ${NRZ} run build1 --filter=app-b --graph
...
Can't depend on unknown tasks
  $ ${NRZ} run build2 > BUILD2 2>&1
  [1]
  $ cat BUILD2 | grep --only-match 'x Could not find "app-a#custom" in root nrz.json or "custom" in package'
  x Could not find "app-a#custom" in root nrz.json or "custom" in package

Can't depend on tasks from unknown packages
  $ ${NRZ} run build3 > BUILD3 2>&1
  [1]
