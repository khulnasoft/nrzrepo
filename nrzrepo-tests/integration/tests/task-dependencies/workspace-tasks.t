...
Can't depend on a missing root task
  $ ${NRZ} run build3 --graph > BUILD3 2>&1
  [1]
  $ cat BUILD3 | grep --quiet --only-match 'x //#not-exists requires an entry in nrz.json before it can be depended on'
