...
# The missing-workspace-config-task-with-deps configures dependsOn in the root nrz.json.
...
# 1. First run, assert that dependet tasks run `dependsOn`
  $ ${NRZ} run missing-workspace-config-task-with-deps --filter=missing-workspace-config > tmp.log
...
# can run in non-deterministic order. So we need to validate the logs in the pieces.
  ...
  missing-workspace-config:missing-workspace-config-task-with-deps: cache miss, executing 9ed8628911841548
  ...
  missing-workspace-config:missing-workspace-config-underlying-task: cache miss, executing 2e1bc7bd383d2a4c
  ...
  blank-pkg:missing-workspace-config-underlying-topo-task: cache miss, executing fdf3beab0e89d32b
