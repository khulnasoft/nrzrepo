...
# The override-values-task-with-deps configures dependsOn in the root nrz.json.
# The workspace does not have a nrz.json config. This test checks that both regular dependencies
# and Topological dependencies are retained from the root config.

# Run override-values-task-with-deps. In the root nrz.json it has two dependsOn values
...
# top level task "override-values-task-with-deps" should run. None of the dependencies should run.
  $ ${NRZ} run override-values-task-with-deps --filter=override-values
...
# This is the same test as above, but with --dry and testing the resolvedTaskDefinition has the same value for dependsOn
  $ ${NRZ} run override-values-task-with-deps --filter=override-values --dry=json | jq '.tasks | map(select(.taskId == "override-values#override-values-task-with-deps")) | .[0].resolvedTaskDefinition'
  {
    "outputs": [],
    "cache": true,
    "dependsOn": [],
...
# This task is similar, but `dependsOn` in the root nrz.json _only_ has a topological dependency
# This test was written to validate a common case of `build: dependsOn: [^build]`
  $ ${NRZ} run override-values-task-with-deps-2 --filter=override-values --dry=json | jq '.tasks | map(select(.taskId == "override-values#override-values-task-with-deps-2")) | .[0].resolvedTaskDefinition'
  {
    "outputs": [],
    "cache": true,
    "dependsOn": [],
