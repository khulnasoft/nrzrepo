...
# The override-values-task task in the root nrz.json has ALL the config. The workspace config
...
# 1. First run, assert that the right `outputs` are cached.
  $ ${NRZ} run override-values-task --filter=override-values > tmp.log
  $ cat tmp.log
  ...
  apps/override-values/.nrz/nrz-override-values-task.log
...
2. Run again and assert cache hit, and that full output is displayed
  $ ${NRZ} run override-values-task --filter=override-values
  \xe2\x80\xa2 Packages in scope: override-values (esc)
  \xe2\x80\xa2 Running override-values-task in 1 packages (esc)
  \xe2\x80\xa2 Remote caching disabled (esc)
...
3a. Change a file that is declared as input in root config, and assert cache hit and FULL NRZ
...
4. Set env var and assert cache miss, and that hash is different from above
  $ OTHER_VAR=somevalue ${NRZ} run override-values-task --filter=override-values
  \xe2\x80\xa2 Packages in scope: override-values (esc)
  \xe2\x80\xa2 Running override-values-task in 1 packages (esc)
  \xe2\x80\xa2 Remote caching disabled (esc)
...
4a. Set env var that is declared in root config, and assert cache hit and FULL NRZ
  $ OTHER_VAR=somevalue ${NRZ} run override-values-task --filter=override-values
  \xe2\x80\xa2 Packages in scope: override-values (esc)
  \xe2\x80\xa2 Running override-values-task in 1 packages (esc)
  \xe2\x80\xa2 Remote caching disabled (esc)
