...
const { nrzGradient, nrzLoader, info, error, warn } = logger;
...
function trackOptions(opts: CreateCommandOptions) {
  ...
  opts.telemetry?.trackOptionNrzVersion(opts.nrzVersion);
...
export async function create(
  ...
  tryGitInit(root, `feat(create-nrz): create ${exampleName}`);
    ...
    for (const transform of transforms) {
      try {
        ...
        if (transformResult.result === "success") {
          ...
          tryGitAdd();
          tryGitCommit(
            `feat(create-nrz): apply ${transformResult.name} transform`
          );
        }
  ...
  info("Creating a new Nrzrepo with:");
  logger.log();
  ...
  if (hasPackageJson && !skipInstall) {
    ...
    if (
      ...
      const loader = nrzLoader("Installing dependencies...").start();
      await install({
        project,
      ...
      tryGitCommit("feat(create-nrz): install dependencies");
      loader.stop();
    }
  }
  ...
  if (projectDirIsCurrentDir) {
    logger.log(
      `${picocolors.bold(
        nrzGradient(">>> Success!")
      )} Your new Nrzrepo is ready.`
    );
  } else {
    logger.log(
      `${picocolors.bold(
        nrzGradient(">>> Success!")
      )} Created your Nrzrepo at ${picocolors.green(relativeProjectDir)}`
    );
  }
  ...
  if (packageManagerMeta && hasPackageJson) {
    ...
    logger.log(
      `- Enable Remote Caching (recommended): ${picocolors.cyan(
        `${packageManagerMeta.executable} nrz login`
      )}`
    );
    logger.log("   - Learn more: https://turbo.build/repo/remote-cache");
    logger.log();
    logger.log("- Run commands with Nrzrepo:");
    availableScripts
