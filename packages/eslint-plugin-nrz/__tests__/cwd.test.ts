...
describe("eslint cache is busted", () => {
  ...
  it("catches a lint error after changing config", () => {
      ...
      expect(outputJson).toMatchObject([
        {
          messages: [
            {
              message:
                "NONEXISTENT is not listed as a dependency in nrz.json",
            },
          ],
        },
      ]);
    ...
    const nrzJson = readJson<Schema>("nrz.json");
    if (nrzJson && "globalEnv" in nrzJson) {
      nrzJson.globalEnv = ["CI", "NONEXISTENT"];
      write("nrz.json", stringify(nrzJson, null, 2));
    }
