...
import { type PackageJson, logger, searchUp } from "@nrz/utils";
import { frameworks } from "@nrz/types";
...
const meta: Rule.RuleMetaData = {
  ...
  docs: {
    description:
      "Do not allow the use of `process.env` without including the env key in any nrz.json",
    category: "Configuration Issues",
    recommended: true,
    url: `https://github.com/khulnasoft/nrzrepo/tree/main/packages/eslint-plugin-nrz/docs/rules/${RULES.noUndeclaredEnvVars}.md`,
  },
  schema: [
    {
      type: "object",
...
/**
 * Nrzrepo does some nice framework detection based on the dependencies in the package.json.  This function ports that logic to this ESLint rule.
 *
...
function create(context: RuleContextWithOptions): Rule.RuleListener {
  ...
  const hasWorkspaceConfigs = project.projectWorkspaces.some(
    (workspaceConfig) => Boolean(workspaceConfig.nrzConfig)
  );
  ...
  const checkKey = (node: Node, envKey?: string) => {
    ...
    let message = `{{ envKey }} is not listed as a dependency in ${
      hasWorkspaceConfigs ? "root nrz.json" : "nrz.json"
    }`;
    if (workspaceConfig?.nrzConfig) {
      if (cwd) {
        // if we have a cwd, we can provide a relative path to the workspace config
        message = `{{ envKey }} is not listed as a dependency in the root nrz.json or workspace (${path.relative(
          cwd,
          workspaceConfig.workspacePath
        )}) nrz.json`;
      } else {
        message = `{{ envKey }} is not listed as a dependency in the root nrz.json or workspace nrz.json`;
      }
    }
