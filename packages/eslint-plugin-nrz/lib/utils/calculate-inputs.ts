...
import type { WorkspaceConfig } from "@nrz/utils";
import { getWorkspaceConfigs } from "@nrz/utils";
import type { PipelineV1, RootSchemaV1, RootSchemaV2 } from "@nrz/types";
import { forEachTaskDef } from "@nrz/utils/src/getNrzConfigs";
...
function getTaskAddress(taskName: string): {
  ...
  if (taskName.length === 0) {
    throw new Error("Invalid task name found in nrz.json.");
  }
...
export class Project {
  ...
  generateKey(): ProjectKey {
    ...
    if (this.projectRoot?.nrzConfig && !("extends" in this.projectRoot)) {
      const rootNrzJson = this.projectRoot;

      global = processGlobal(
        this.projectRoot.workspacePath,
        this.projectRoot.nrzConfig
      );
    ...
    this.projectWorkspaces.forEach((projectWorkspace) => {
      if (!projectWorkspace.nrzConfig) {
        return;
      }

      forEachTaskDef(
        projectWorkspace.nrzConfig,
        ([taskName, taskDefinition]) => {
          const { workspaceName: erroneousWorkspaceName, scriptName } =
            getTaskAddress(taskName);
          if (erroneousWorkspaceName) {
            throw new Error(
              "May not specify workspace name in non-root nrz.json"
            );
          }
