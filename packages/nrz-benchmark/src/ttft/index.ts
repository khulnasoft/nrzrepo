...
import { NRZ_BIN, type TTFTData } from "../helpers";
import { run } from "./run";
...
// process.cwd() should be packages/nrz-benchmark/ here.
...
if (!fs.existsSync(NRZ_BIN)) {
  throw new Error("No nrz binary found");
}

cp.execSync(`${NRZ_BIN} --version`, { stdio: "inherit" });
...
for (const item of profileJSON) {
  ...
  if (args.nrz_version) {
    ttftData.nrzVersion = `${args.nrz_version}`;
  }
...
function printUsageMessage() {
  console.log("Usage:\n\npnpm -F @nrz/benchmark ttft <path>");
}
