import { convertCase, logger } from "@nrz/utils";
import { workspace, type NrzGeneratorCLIOptions } from "../workspace";
 ...
 *
 * Used by the nrz Rust cli to handoff commands to the \@nrz/gen binary
 */
export async function raw(command: string, options: { json: string }) {
  if (command === "workspace") {
    ...
    await workspace(parsedArgs as NrzGeneratorCLIOptions);
  } else if (command === "run") {
