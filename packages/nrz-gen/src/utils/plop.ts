...
import { searchUp, getNrzConfigs, logger } from "@nrz/utils";
import { GeneratorError } from "./error";

const SUPPORTED_CONFIG_EXTENSIONS = ["ts", "js", "cjs"];
const NRZ_GENERATOR_DIRECTORY = path.join("nrz", "generators");
...
const SUPPORTED_WORKSPACE_GENERATOR_CONFIGS = SUPPORTED_CONFIG_EXTENSIONS.map(
  (ext) => path.join(NRZ_GENERATOR_DIRECTORY, `config.${ext}`)
);
...
function injectNrzrepoData({
  ...
  let nrzConfigs = {};
  try {
    nrzConfigs = getNrzConfigs(generator.basePath);
  } catch (e) {
    // ignore
  }
...
export async function runCustomGenerator({
  ...
  const results = await gen.runActions(
    { ...answers, ...injectNrzrepoData({ project, generator: gen }) },
    {
