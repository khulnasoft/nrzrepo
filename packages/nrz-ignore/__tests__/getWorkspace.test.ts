...
describe("getWorkspace()", () => {
  ...
  it("getWorkspace used current directory if not specified", () => {
    expect(getWorkspace({})).toEqual("nrz-ignore");
    expect(mockConsole.log).toHaveBeenCalledWith(
      "≫  ",
      'Inferred "nrz-ignore" as workspace from "package.json"'
    );
  });
  ...
  it("getWorkspace returns null when no arg is provided and package.json can be found", () => {
    ...
    expect(mockConsole.error).toHaveBeenCalledWith(
      "≫  ",
      '"__fixtures__/no-app/package.json" could not be found. nrz-ignore inferencing failed'
    );
  });
});
