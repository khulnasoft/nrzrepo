...
import { NrzIgnoreTelemetry, TelemetryConfig } from "@nrz/telemetry";
import { nrzIgnore } from "../src/ignore";
...
describe("nrzIgnore()", () => {
  ...
  const telemetry = new NrzIgnoreTelemetry({
  ...
  it("throws error and allows build when exec fails", () => {
    ...
    nrzIgnore("test-workspace", { telemetry });

    expect(mockExec).toHaveBeenCalledWith(
      `npx -y nrz@^2 run build --filter="test-workspace...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
  ...
  it("throws pretty error and allows build when exec fails", () => {
    ...
    expect(mockConsole.warn).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      `nrz-ignore could not complete - no package manager detected, please commit a lockfile, or set "packageManager" in your root "package.json"`
    );
  ...
  it("throws pretty error and allows build when can't find previous sha", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx -y nrz@^2 run build --filter="test-workspace...[too-far-back]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    ...
    expect(mockConsole.warn).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      `nrz-ignore could not complete - a ref or SHA is invalid. It could have been removed from the branch history via a force push, or this could be a shallow clone with insufficient history`
    );
  ...
  it("throws pretty error and allows build when fallback fails", () => {
    ...
    nrzIgnore("test-workspace", { fallback: "HEAD^" });
    ...
    expect(mockConsole.warn).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      `nrz-ignore could not complete - parent commit does not exist or is unreachable`
    );
  ...
  it("skips checks and allows build when no workspace can be found", () => {
    nrzIgnore(undefined, { directory: "__fixtures__/no-app" });
    expect(mockConsole.error).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      expect.stringContaining(
        " could not be found. nrz-ignore inferencing failed"
      )
    );
  ...
  it("skips checks and allows build when a workspace with no name is found", () => {
    nrzIgnore(undefined, { directory: "__fixtures__/invalid-app" });
    expect(mockConsole.error).toHaveBeenNthCalledWith(
      1,
      "≫  ",
  ...
  it("skips checks and allows build when no monorepo root can be found", () => {
    nrzIgnore(undefined, { directory: "/" });
    expectBuild(mockExit);
    expect(mockConsole.error).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      "Monorepo root not found. nrz-ignore inferencing failed"
    );
  });

  it("skips checks and allows build when NRZ_FORCE is set", () => {
    process.env.NRZ_FORCE = "true";
    nrzIgnore(undefined, { directory: "test-workspace" });
  ...
  it("allows build when no comparison is returned", () => {
    ...
    process.env.VERCEL_GIT_COMMIT_REF = "my-branch";
    nrzIgnore("test-app", { directory: "__fixtures__/app" });
    expect(mockConsole.log).toHaveBeenNthCalledWith(
      5,
      "≫  ",
  ...
  it("skips build for `previousDeploy` comparison with no changes", () => {
    ...
    nrzIgnore(undefined, { directory: "__fixtures__/app" });
    validateLogs(mockConsole.log, [
      [
        "≫  ",
        "Using Nrzrepo to determine if this project is affected by the commit...\n",
      ],
      ...
      [
        "≫  ",
        'Analyzing results of `npx -y nrz@^2 run build --filter="test-app...[last-deployed-sha]" --dry=json`',
      ],
  ...
  it("allows build for `previousDeploy` comparison with changes", () => {
    ...
    validateLogs(mockConsole.log, [
      ...
      [
        "≫  ",
        'Analyzing results of `npx -y nrz@^2 run "workspace#build" --filter="test-app...[last-deployed-sha]" --dry=json`',
      ],
  ...
  it("allows build for unavailable `previousDeploy` comparison with fallback", () => {
    ...
    validateLogs(mockConsole.log, [
      ...
      [
        "≫  ",
        'Analyzing results of `npx -y nrz@^2 run "workspace#build" --filter="test-app...[HEAD^2]" --dry=json`',
      ],
      ["≫  ", 'This commit affects "test-app"'],
      [expect.stringContaining("✓ Proceeding with deployment")],
    ]);
  ...
  it("throws error and allows build when json cannot be parsed", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx -y nrz@^2 run build --filter="test-app...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    ...
    expect(mockConsole.error).toHaveBeenNthCalledWith(
      1,
      "≫  ",
      'Failed to parse JSON output from `npx -y nrz@^2 run build --filter="test-app...[HEAD^]" --dry=json`.'
    );
  ...
  it("runs full nrz-ignore check when commit message contains a conflicting string", () => {
    process.env.VERCEL = "1";
  ...
  it("passes max buffer to nrz execution", () => {
    const mockExec = jest
      .spyOn(child_process, "exec")
      .mockImplementation((command, options, callback) => {
        if (callback) {
    ...
    nrzIgnore(undefined, { directory: "__fixtures__/app", maxBuffer: 1024 });
  ...
  it("defaults to latest nrz if no hints for version", () => {
    const mockExec = jest
      .spyOn(child_process, "exec")
      .mockImplementation((command, options, callback) => {
        if (callback) {
    ...
    nrzIgnore(undefined, { directory: "__fixtures__/invalid_nrz_json" });

    expect(mockExec).toHaveBeenCalledWith(
      `npx -y nrz run build --filter="test-app...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
