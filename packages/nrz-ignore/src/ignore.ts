...
import { getNrzRoot } from "@nrz/utils";
import type { DryRun } from "@nrz/types";
import { getComparison } from "./getComparison";
import { getTask } from "./getTask";
import { getWorkspace } from "./getWorkspace";
import { getNrzVersion } from "./getNrzVersion";
import { log, info, warn, error } from "./logger";
import { shouldWarn } from "./errors";
import type { NrzIgnoreArg, NrzIgnoreOptions } from "./types";
import { checkCommit } from "./checkCommit";

function trackOptions(opts: NrzIgnoreOptions) {
...
export function nrzIgnore(
  workspaceArg: NrzIgnoreArg,
  opts: NrzIgnoreOptions
) {
  ...
  // check for NRZ_FORCE and bail early if it's set
  if (process.env.NRZ_FORCE === "true") {
    info("`NRZ_FORCE` detected");
    return continueBuild();
  }

  // find the monorepo root
  const root = getNrzRoot(inputs.directory);
  if (!root) {
    error("Monorepo root not found. nrz-ignore inferencing failed");
    return continueBuild();
  }
  ...
  // Find the version of nrz this project uses
  const nrzVersion = getNrzVersion(inputs, root);
  ...
  // If we can't find a nrz version in package.json, don't specify a version
  const nrz = nrzVersion ? `nrz@${nrzVersion}` : "nrz";
  // Build, and execute the command
  const command = `npx -y ${nrz} run ${task} --filter="${workspace}...[${comparison.ref}]" --dry=json`;
