...
describe("TelemetryClient", () => {
  ...
  it("sends request when batch size is reached", (t) => {
    ...
    assert.deepEqual(
      ...
      "https://example.com/api/nrzrepo/v1/events"
    );
    ...
    assert.equal(
      "x-nrz-session-id" in mockPost.mock.calls[0].arguments[1].headers,
      true
    );
    assert.equal(
      "x-nrz-telemetry-id" in mockPost.mock.calls[0].arguments[1].headers,
      true
    );
    assert.equal(
      /create-nrz 1\.0\.0/.test(
