...
const DEBUG_ENV_VAR = "NRZ_TELEMETRY_DEBUG";
const DISABLED_ENV_VAR = "NRZ_TELEMETRY_DISABLED";
const DISABLED_MESSAGE_ENV_VAR = "NRZ_TELEMETRY_MESSAGE_DISABLED";
...
/**
 * NOTE: This package is a direct port of the telemetry config struct from the nrz-telemetry crate. Any changes
 * made here should be reflected in the nrz-telemetry crate as well.
 *
 * https://github.com/khulnasoft/nrzrepo/blob/main/crates/nrzrepo-telemetry/src/config.rs
 */
export class TelemetryConfig {
  ...
  isEnabled(): boolean {
    ...
    const nrzTelemetryDisabled = process.env[DISABLED_ENV_VAR] || "0";

    if (
      doNotTrack === "1" ||
      doNotTrack.toLowerCase() === "true" ||
      nrzTelemetryDisabled === "1" ||
      nrzTelemetryDisabled.toLowerCase() === "true"
    ) {
      return false;
    }
  ...
  isTelemetryWarningEnabled(): boolean {
    const nrzTelemetryMsgDisabled =
      process.env[DISABLED_MESSAGE_ENV_VAR] || "0";

    const isDisabled =
      nrzTelemetryMsgDisabled === "1" ||
      nrzTelemetryMsgDisabled.toLowerCase() === "true";
