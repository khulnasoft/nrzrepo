...
export interface BaseSchema {
  ...
  $schema?: string;
  /**
   * An object representing the task dependency graph of your project. nrz interprets
  ...
  tasks: {
    /**
     * The name of a task that can be executed by nrz. If nrz finds a workspace
...
/** A `nrz.json` file in a package in the monorepo (not the root) */
export interface WorkspaceSchema extends BaseSchema {
  /**
   * This key is only available in Workspace Configs
   * and cannot be used in your root nrz.json.
   *
   * Tells nrz to extend your root `nrz.json`
...
export interface RootSchema extends BaseSchema {
  ...
  /**
   * Configuration options that control how nrz interfaces with the remote cache.
   *
   ...
   *
   * This is highly discouraged as it leaves `nrz` dependent on system
   * configuration to infer the correct package manager.
   *
   * Some nrz features are disabled if this is set to true.
   *
   * @defaultValue `false`
   */
...
export interface Pipeline {
   ...
   *
   * Prefixing an item in dependsOn with a ^ prefix tells nrz that this task depends
   ...
   *
   * Note: If you are migrating from a nrz version 1.5 or below,
   ...
   *
   * Nrzrepo captures task logs for all tasks. This enables us to cache tasks whose runs
   ...
   * nrz that this is a long-running task and will ensure that other tasks
   * cannot depend on it.
   *
   ...
   * `nrz watch` watches for changes to your packages and automatically
...
export interface RemoteCache {
   ...
   * `true`, Nrzrepo will sign every uploaded artifact using the value of the environment
   * variable `NRZ_REMOTE_CACHE_SIGNATURE_KEY`. Nrzrepo will reject any downloaded artifacts
  ...
  /**
   * Indicates if the remote cache is enabled. When `false`, Nrzrepo will disable
