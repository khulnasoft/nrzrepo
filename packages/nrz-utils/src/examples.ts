...
export async function getRepoInfo(
  ...
  if (
    // Support repos whose entire purpose is to be a Nrzrepo example, e.g.
    // https://github.com/:username/:my-cool-nrzrepo-example-repo-name.
    tree === undefined ||
    // Support GitHub URL that ends with a trailing slash, e.g.
    // https://github.com/:username/:my-cool-nrzrepo-example-repo-name/
    // In this case "t" will be an empty string while the nrz part "_branch" will be undefined
...
export async function downloadAndExtractExample(root: string, name: string) {
  const tempFile = await downloadTar(
    `https://codeload.github.com/khulnasoft/nrzrepo/tar.gz/main`,
    `nrz-ct-example`
  );
