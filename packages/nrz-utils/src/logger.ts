...
export const nrzGradient = gradient(BLUE, RED);
export const nrzBlue = hex(BLUE);
export const nrzRed = hex(RED);
export const yellow = hex(YELLOW);

export const nrzLoader = (text: string) =>
  ora({
    text,
    spinner: {
      frames: ["   ", nrzBlue(">  "), nrzBlue(">> "), nrzBlue(">>>")],
    },
  });

export const info = (...args: Array<unknown>) => {
  log(nrzBlue(pcBold(">>>")), args.join(" "));
};
...
export const error = (...args: Array<unknown>) => {
  ...
  console.error(nrzRed(pcBold(">>>")), args.join(" "));
};
